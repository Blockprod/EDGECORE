...
v31b APPROACH: "Quality from wide universe" — keep 117-symbol funnel but
tighten quality filters and ACTIVATE momentum overlay as entry gate:
  ...
  FIX 2: Allocation 35%/pair (was 25%) — winners generate meaningful P&L
  FIX 3: entry_z=1.7 (was 1.6) — higher quality signals
  FIX 4: corr=0.65 (was 0.60) — only truly related pairs
  FIX 5: FDR q=0.25 (was 0.30) — tighter statistical significance
  FIX 6: Heat=3.5 (was 4.0) — ~10 concurrent positions max
  FIX 7: Blacklist 4/10 (was 6/7) — kill serial losers faster
  FIX 8: max_positions=15 (was 20) — controlled exposure
  FIX 9: Weekly gate=0.25 (was 0.2) — slightly tighter confirmation
...
# == Universe (117 symbols — unchanged from v31) ===========================
SYMBOLS = [
    "SPY",
    # Technology (Mega Cap)
...
# Blacklist (tighter — kill serial losers faster)
...
def main():
    ...
    print("  FIX 1: MomentumOverlay ACTIVE in backtest path (was dead code)")
    ...
    print("  FIX 8: Max positions=15 (was 20)")
    print(f"  FIX 9: Weekly gate={WEEKLY_Z_GATE} (was 0.2)")
    print("  Momentum:     lookback=20, weight=0.30, min_str=0.30")
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write("Momentum: ACTIVE, lookback=20, weight=0.30, gate=contra-reject\n")
