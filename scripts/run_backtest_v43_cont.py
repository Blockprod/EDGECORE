...
V43A_RESULT = ("v43a", 0.22, -1.56, 1, 0, 4, "FAIL", "Regime ON + fdr_q=0.30 + vol_th=0.35 (primary fix)")
...
def _run_all_windows(runner, ts20, regime_on, fdr_q, trend_fav_sz, vol_threshold):
    ...
    results = []
    for label, train_start, _, oos_start, oos_end in WF_WINDOWS:
...
def _print_config_results(cfg_label, description, results):
    ...
    for label, sh, v, ret, wr, t, dd, elapsed, err in results:
        if err:
            ...
            tpy = t * 2
            print(
                "    %-12s  S=%5.2f  %+6.2f%%  WR=%5.1f%%  t=%2d(~%2.0f)"
                "  DD=%+6.2f%%  [%s/%ds]" % (label, sh, ret, wr, t, tpy, dd, v, elapsed)
            )
    ...
    if passes >= 4:
        ...
        verdict = "FAIL"
    print(
        "  Summary: PASS=%d/5  S-PASS=%d/5  FAIL=%d/5 |"
        " Sharpe avg=%.2f  min=%.2f  -> %s" % (passes, spasses, fails, avg_sh, min_sh, verdict)
    )
...
def main():
    ...
    for cfg_label, avg_sh, min_sh, passes, spasses, fails, verdict in all_summaries:
        print(
            "  %-6s  %8.2f  %8.2f  %5d  %6d  %6d  %-8s" % (cfg_label, avg_sh, min_sh, passes, spasses, fails, verdict)
        )
        if avg_sh > best_avg:
            best_avg = avg_sh
            best_cfg = cfg_label
    ...
    if best:
        ...
        if verdict == "PASS":
            ...
            print(f"  Freeze {best_cfg} params as v43_frozen.")
        elif verdict == "S-PASS":
            print("  NEXT: Test lower leverage (2.0x) or wider train windows (24m).")
            print(f"  Freeze {best_cfg} as v43_frozen candidate with OOS degradation note.")
        else:
