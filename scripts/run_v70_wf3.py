#!/usr/bin/env python
"""v70 Walk-Forward 3-Window — Validation CERT-03b.

OBJECTIF : Filtrer les paires à retour lent avec max_half_life=40j (au lieu de 60j).
  Rationale : les paires avec HL>40j (spread se referme en >6 semaines) sont
  exposées aux cassures structurelles macro (2025 : tarifs Trump, rotations).
  En n'autorisant que HL<=40j, on sélectionne uniquement les paires qui
  se referment avant qu'une cassure de régime ait le temps de se propager.

RÉSULTATS WF3 DE RÉFÉRENCE :
  v63 : W1=0.90  W2=0.91  W3=-0.14  Combined=0.5554  CERT=PASS  (max_hl=60)
  v66 : W1=0.22  W2=0.03  W3=+1.13  Combined=0.4578  FAIL       (lookback court)
  v69 : W1=0.90  W2=0.91  W3=-0.14  Combined=0.5554  CERT=PASS  (z_stop=3.0, neutre)

PARAMÈTRES v70 vs v63 :
  MODIFIE : max_half_life=40j  (était 60j)
  INCHANGÉ: lookback=180, additional=[63], entry_z=1.8, exit_z=0.10
            z_score_stop=4.0, adf_p=0.15, min_corr=0.65, max_loss=5%
            time_stop=45d, blacklist ON(2/30), regime unfav_sz=0.30, momentum OFF
...
Usage:
    venv\\Scripts\\python.exe scripts/run_v70_wf3.py
"""
...
def _apply_settings_v70():
    """v70 — v63 base + max_half_life réduit à 40j."""
    s = get_settings()
    ...
    # ── Coïntégration : max_half_life RÉDUIT ─────────────────────────────
    ...
    s.strategy.max_half_life = 40  # v70: 40j (était 60j en v63)
...
def main():
    print("=" * 70)
    print("  v70 Walk-Forward 3-Window — Validation CERT-03b")
    print("  max_half_life=40j  (v63 base, seul changement)")
    print("  Rationale : HL<=40j = spread se referme en ~6 semaines")
    print("             -> immunise contre les cassures macro de 2-3 mois (2025)")
    print()
    ...
    print("=" * 70)
    print("  CERT-03b AGGREGATE RESULTS — v70")
    print("=" * 70)
    print(f"  Windows completed  : {n_valid}/{len(WF_WINDOWS)}")
    print(f"  Total OOS trades   : {total_trades}")
    print(f"  Combined Sharpe    : {combined_sh:.4f}   (v63: 0.5554)")
    ...
    # ── Save results ──────────────────────────────────────────────────────
    results_path = os.path.join(results_dir, "v70_wf3_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        f.write("v70 Walk-Forward 3-Window — Validation CERT-03b\n")
        f.write("=" * 60 + "\n")
        f.write("MODIFIE vs v63 : max_half_life=40j  (était 60j)\n")
        f.write("INCHANGE vs v63: lookback=180, additional=[63]\n")
        f.write("  entry_z=1.8, exit_z=0.10, z_score_stop=4.0, adf_p=0.15\n")
        f.write("  min_corr=0.65, max_loss=5%, time_stop=45d\n")
        ...
        f.write(f"Combined Sharpe  : {combined_sh:.4f}   (v63: 0.5554)\n")
