#!/usr/bin/env python
"""EDGECORE Backtest v31f -- Win/Loss Ratio Optimization (TimeStop FIXED).

ITERATION HISTORY:
  v30b: +5.25%, Sharpe 0.74,  25 trades, WR 60%,  PF 2.63, DD -2.6%
  v31d: -0.69%, Sharpe -0.03, 50 trades, WR 60%,  PF 0.96, DD -9.4%
  v31e: (running -- exit_z=0.3 + stop=6%, but time_stop BUG: dataclass
         class-attribute mutation does NOT change __init__ defaults)

v31f = v31e WITH TimeStopConfig bug FIXED:
  - Properly instantiates TimeStopConfig(half_life_multiplier=1.5, max_days_cap=30)
  - Passes TimeStopManager to runner.run_unified(time_stop=...) via new param
  - All v31e changes retained: exit_z=0.3, stop_pct=6%, max_HL=55

BUG FIX: In v31e, we did:
    TimeStopConfig.half_life_multiplier = 1.5   # sets CLASS attr
    # but TimeStopConfig() still uses __init__ default=2.0!
  Now we do:
    ts = TimeStopManager(TimeStopConfig(half_life_multiplier=1.5, max_days_cap=30))
    runner.run_unified(..., time_stop=ts)
...
# == Curated Universe (same as v31d/e - 74 symbols) ========================
SYMBOLS = [
    "SPY",
    # Technology (Mega Cap)
...
# == v31f Parameters (= v31e but with WORKING time_stop) ====================
ENTRY_Z = 1.8           # v30b proven
EXIT_Z = 0.3            # let winners run further (was 0.5)
ALLOC_PCT = 50.0        # v30b proven
HEAT = 3.0              # v30b proven
STOP_PCT = 0.06         # tighter tail losses (was 0.07)
MIN_CORR = 0.65         # v30b proven
MAX_HALF_LIFE = 55      # only fast-converging pairs (was 60)
...
# Time stop — PROPERLY CONFIGURED via TimeStopConfig instance
TIME_STOP_MULT = 1.5    # cut stale trades 25% sooner (was 2.0)
MAX_HOLD_DAYS = 30       # absolute time cap tighter (was 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="EDGECORE Backtest v31f -- Win/Loss Ratio (TimeStop FIXED)"
    )
    ...
    # == TIME STOP: Properly instantiate via constructor (FIXES dataclass bug) ==
    ...
    print("=" * 70)
    print("  EDGECORE BACKTEST v31f -- Win/Loss Ratio (TimeStop FIXED)")
    print("=" * 70)
    ...
    print("  --- v31f TimeStop FIX ---")
    print(f"  exit_z:       {EXIT_Z} (was 0.5 -- winners run further)")
    print(f"  time_stop:    {TIME_STOP_MULT}x HL, cap={MAX_HOLD_DAYS}d (NOW WORKING)")
    print(f"  stop_pct:     {STOP_PCT*100:.0f}% (was 7% -- tighter tail cap)")
    print(f"  max_HL:       {MAX_HALF_LIFE} (was 60)")
    print("  Momentum:     STRICT (blocks ALL contra-momentum)")
    print("=" * 70)
    print()

    t0 = time.time()
    print("[EDGECORE] Loading IBKR data + running backtest v31f...")
    print()
    ...
    print(f"\n[EDGECORE] Backtest v31f completed in {elapsed:.1f}s")
    print()
    ...
    out_path = os.path.join(_ROOT, "results", "bt_results_v31f.txt")
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("EDGECORE Backtest v31f -- Win/Loss Ratio (TimeStop FIXED)\n")
        f.write(f"{'=' * 60}\n")
        ...
        f.write("Momentum: STRICT (blocks ALL contra-momentum)\n")
