#!/usr/bin/env python
"""v64 — Walk-Forward 3-Window — v63 base (PASS) + tighter risk controls (CERT-03b).

STRATEGY: v63 PASSED CERT-03b (Combined Sharpe 0.5554) but W3 was still negative (-0.14).
v64 blends the best of v62 (tight stops) and v63 (blacklist):

1. BLACKLIST ON (from v63 — the key breakthrough)
   → max_consecutive_losses=2, cooldown=30d

2. entry_z = 1.9 (was 1.8 in v63, 2.0 in v62)
   → Slightly more selective than v63 to improve trade quality

3. exit_z = 0.15 (was 0.10 in v63, 0.20 in v62)
   → Balance: let winners run a bit more than v63

4. max_loss = 4% (was 5% in v63, 3% in v62)
   → Tighter stop-loss to cut W3 losers faster

5. time_stop = 38d (was 45d in v63, 30d in v62)
   → Shorter holding period reduces exposure in W3 regime shifts

6. unfav_sz = 0.25 (was 0.30 in v63, 0.25 in v62)
   → Size down more in unfavorable regime
...
def _apply_settings_v64():
    """v64 — v63 base (PASS) + tighter risk controls."""
    s = get_settings()

    # ── Signal: Slightly higher entry Z for quality ─────────────────────
    s.strategy.entry_z_score = 1.9  # v64: 1.9 (was 1.8 in v63)
    s.strategy.exit_z_score = 0.15  # v64: 0.15 (was 0.10 in v63)
    s.strategy.z_score_stop = 4.0
    s.strategy.entry_z_min_spread = 0.0

    # ── Cointegration: same as v63 ──────────────────────────────────────
    ...
    # ── Sizing: tighter max loss ────────────────────────────────────────
    s.strategy.max_position_loss_pct = 0.04  # v64: 4% (was 5% in v63)
    ...
    # ── REGIME: slightly lower unfavorable sizing ───────────────────────
    s.regime.enabled = True
    s.regime.neutral_sizing = 1.0
    s.regime.trend_favorable_sizing = 1.0
    s.regime.trend_unfavorable_sizing = 0.25  # v64: 0.25 (was 0.30 in v63)
    # neutral_band_pct = 0.02 (default) — keep v63's value

    # ── Momentum — disabled ─────────────────────────────────────────────
    s.momentum.enabled = False

    # ── BLACKLIST ON — keep v63's breakthrough ───────────────────────────
...
def _run_window(
    ...
    ts = TimeStopManager(
        TimeStopConfig(
            ...
            max_days_cap=38,  # v64: 38d (was 45d in v63)
            default_max_bars=38,  # v64: 38d
        )
    )
    ...
    try:
        metrics = runner.run_unified(
            ...
            max_position_loss_pct=0.04,  # v64: 4% (was 5% in v63)
...
def main():
    print("=" * 70)
    print("  v64 — v63 base (PASS) + tighter risk controls")
    print("  entry_z=1.9, exit_z=0.15, max_loss=4%, time_stop=38d")
    print("  blacklist ON(2/30), REGIME ON: neutral_band=0.02, unfav_sz=0.25")
    print("=" * 70)
    print()
    ...
    # ── Save results ──────────────────────────────────────────────────────
    results_path = os.path.join(results_dir, "v64_wf3_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        f.write("v64 Walk-Forward 3-Window — v63 base (PASS) + tighter risk controls — CERT-03b\n")
        f.write("=" * 60 + "\n")
        f.write("APPROACH: v63 base + entry_z=1.9, exit_z=0.15, max_loss=4%, time_stop=38d, unfav_sz=0.25\n")
