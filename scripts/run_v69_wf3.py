#!/usr/bin/env python
"""v69 Walk-Forward 3-Window — Validation CERT-03b.

OBJECTIF : Tester un stop de divergence plus serré (z_score_stop=3.0 au lieu de 4.0).
  Rationale : les pertes majeures (EOG_PSX -2938$, SLB_MPC -1161$) correspondent
  à des spreads continuant de diverger au-delà de 4σ. Un stop à 3σ les coupe
  ~30% plus tôt sans toucher à la sélection de paires → pas de biais de snooping.

RÉSULTATS WF3 DE RÉFÉRENCE :
  v63 : W1=0.90  W2=0.91  W3=-0.14  Combined=0.5554  CERT=PASS  (stop=4.0)
  v66 : W1=0.22  W2=0.03  W3=+1.13  Combined=0.4578  FAIL       (lookback=120)
  v68 : W1=0.79  W2=-0.39 W3=-1.42  Combined=-0.3406 FAIL       (lookback=150)

PARAMÈTRES v69 vs v63 :
  MODIFIE : z_score_stop=3.0  (était 4.0)
  INCHANGÉ: lookback=180, additional=[63], entry_z=1.8, exit_z=0.10
            adf_p=0.15, min_corr=0.65, max_hl=60j, max_loss=5%
...
Usage:
    venv\\Scripts\\python.exe scripts/run_v69_wf3.py
"""
...
def _apply_settings_v69():
    """v69 — v63 base + z_score_stop serré à 3.0σ."""
    s = get_settings()

    # ── Signal : stop serré ──────────────────────────────────────────────
    s.strategy.entry_z_score = 1.8
    s.strategy.exit_z_score = 0.10
    s.strategy.z_score_stop = 3.0  # v69: 3.0 (était 4.0 en v63)
    s.strategy.entry_z_min_spread = 0.0

    # ── Coïntégration : INCHANGÉ vs v63 ──────────────────────────────────
...
def main():
    print("=" * 70)
    print("  v69 Walk-Forward 3-Window — Validation CERT-03b")
    print("  z_score_stop=3.0  (v63 base, seul changement)")
    print("  lookback=180, additional=[63], entry_z=1.8, exit_z=0.10")
    ...
    print("=" * 70)
    print("  CERT-03b AGGREGATE RESULTS — v69")
    print("=" * 70)
    ...
    # ── Save results ──────────────────────────────────────────────────────
    results_path = os.path.join(results_dir, "v69_wf3_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        f.write("v69 Walk-Forward 3-Window — Validation CERT-03b\n")
        f.write("=" * 60 + "\n")
        f.write("MODIFIE vs v63 : z_score_stop=3.0  (était 4.0)\n")
        f.write("INCHANGE vs v63: lookback=180, additional=[63]\n")
        f.write("  entry_z=1.8, exit_z=0.10, adf_p=0.15\n")
