#!/usr/bin/env python
"""EDGECORE Backtest v31j -- v31h + Moderate Universe Expansion.

ITERATION HISTORY:
  v30b: +5.25%, Sharpe 0.74,  25 trades, WR 60%,  PF 2.63, DD -2.6%
  v31h: +8.17%, Sharpe 1.31,  24 trades, WR 62.5%, PF 3.88, DD -1.79%  *** BEST ***
  v31i: +4.09%, Sharpe 0.50,  28 trades, WR 64.3%, PF 1.51, DD -3.9%
       (lower barriers diluted edge)

v31j = v31h EXACT params + moderate universe expansion:
  - v31h params EXACTLY (entry_z=1.8, exit_z=0.5, momentum strict, etc.)
  - Universe expansion from 37  ~50 symbols:
    * Add sector ETFs (XLF, XLE, XLK)  excellent pair candidates with components
    * Add INTC, QCOM, TXN (proven semis in same tech sector)
    * Add BLK, LLY, TMO (proven mega caps)
    * Add SLB, VLO, MPC (proven energy)
    * Add LMT, MMM (proven industrials)
  - Careful additions: each symbol has a CLEAR sector peer group for pairing

Hypothesis: Adding ~15 carefully selected symbols creates more pairing
opportunities without adding noise. Sector ETFs are especially good partners.
With 422 pairs (74 sym) we got noise; with ~130 pairs (~50 sym) we should
get quality pairs without overwhelming FDR correction.
...
# == Expanded Universe (50 symbols) =========================================
SYMBOLS = [
    "SPY",
    # Technology (v30b + add INTC, QCOM, TXN)
    "AAPL", "MSFT", "GOOGL", "META", "NVDA", "AMD", "AVGO",
    "INTC", "QCOM", "TXN",
    # Financials (v30b + add BLK)
    "JPM", "GS", "BAC", "MS", "WFC", "C", "SCHW", "BLK",
    # Energy (v30b + add SLB, VLO, MPC)
    "XOM", "CVX", "COP", "EOG", "SLB", "VLO", "MPC",
    # Consumer Staples (v30b)
    "KO", "PEP", "PG", "CL", "WMT",
    # Industrials (v30b + add LMT, MMM)
    "CAT", "HON", "DE", "GE", "RTX", "LMT", "MMM",
    # Utilities (v30b)
    "NEE", "DUK", "SO",
    # Healthcare (v30b + add LLY, TMO)
    "JNJ", "PFE", "UNH", "MRK", "ABBV", "LLY", "TMO",
    # Sector ETFs (NEW  excellent pair candidates)
    "XLK", "XLF", "XLE",
]
...
SECTOR_MAP = {
    ...
    "TXN": "technology", "XLK": "technology",
    "JPM": "financials", "GS": "financials", "BAC": "financials",
    "MS": "financials", "WFC": "financials", "C": "financials",
    "SCHW": "financials", "BLK": "financials", "XLF": "financials",
    "XOM": "energy", "CVX": "energy", "COP": "energy", "EOG": "energy",
    "SLB": "energy", "VLO": "energy", "MPC": "energy", "XLE": "energy",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="EDGECORE Backtest v31j -- v31h + Moderate Expansion"
    )
    ...
    # Strategy  v31h params exactly
    ...
    print("=" * 70)
    print("  EDGECORE BACKTEST v31j -- v31h + Moderate Expansion")
    print("=" * 70)
    ...
    t0 = time.time()
    print("[EDGECORE] Loading IBKR data + running backtest v31j...")
    print()
    ...
    print(f"\n[EDGECORE] Backtest v31j completed in {elapsed:.1f}s")
    print()
    ...
    out_path = os.path.join(_ROOT, "results", "bt_results_v31j.txt")
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("EDGECORE Backtest v31j -- v31h + Moderate Expansion\n")
        f.write(f"{'=' * 60}\n")
