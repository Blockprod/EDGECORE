<<<<<<< HEAD
﻿"""Comprehensive backtest diagnostic ÔÇö find all trade blockers."""
...
<<<<<<< HEAD
with open(os.path.join(_ROOT, "results", "backtest_final.log"), encoding="utf-16") as f:
    data = f.read()
...
=======
with open(os.path.join(_ROOT, 'results', 'backtest_final.log'), 'r', encoding='utf-16') as f:
    data = f.read()
...
qual_at_0 = sum(1 for at_str, _pair, z in signals if int(at_str) == 0 and abs(float(z)) >= 1.0)
