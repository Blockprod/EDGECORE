#!/usr/bin/env python
"""EDGECORE Backtest v24 — Moderately Aggressive post-audit backtest.

Same audited infrastructure as v23 but with tuned parameters:
  - entry_z_score: 1.8 (vs 2.0 — catches more opportunities)
  - exit_z_score: 0.5 (unchanged — no whipsaw)
  - allocation: 35% per pair (vs 20% — more capital deployed)
  - max_portfolio_heat: 1.50 (vs 0.95 — allows modest leverage)
  - stop: 8% per position (vs 10% — tighter risk control)
  - min_correlation: 0.65 (vs 0.70 — wider net)
...
# ── v24 Parameter Changes vs v23 ──
ENTRY_Z = 1.8       # v23: 2.0  → More entry opportunities
EXIT_Z = 0.5        # unchanged
ALLOC_PCT = 35.0    # v23: 20%  → More capital per trade
HEAT = 1.50         # v23: 0.95 → Allows modest leverage (150%)
STOP_PCT = 0.08     # v23: 10%  → Tighter stop (8%)
MIN_CORR = 0.65     # v23: 0.70 → Wider pair net
...
def main():
    ...
    print("=" * 70)
    print("  EDGECORE BACKTEST v24 — Moderate Aggressive Parameters")
    print("=" * 70)
    ...
    print("  Rediscovery:  every 5 bar(s)")
    print("=" * 70)
    print()
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("EDGECORE Backtest v24 — Moderate Aggressive\n")
        f.write(f"{'=' * 60}\n")
