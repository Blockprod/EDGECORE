...
def run_config(runner, ts20, cfg_name: str, entry_z: float, description: str) -> list:
    ...
    for label, train_start, _, oos_start, oos_end in WF_WINDOWS:
        ...
        try:
            ...
            print(
                "    %-12s  S=%5.2f  %+6.2f%%  WR=%5.1f%%  t=%2d  DD=%+6.2f%%"
...
def main():
    ...
    print("  " + "-" * 75)
    print(
        "  %-10s  %+6.2f  %+6.2f  %4d  %4d  %4d  %5.1f  %s  [ref: v43a best]"
        % ("v43a", 0.22, -1.56, 1, 0, 4, 3.8, "FAIL")
    )
    print(
        "  %-10s  %+6.2f  %+6.2f  %4d  %4d  %4d  %5.1f  %s  [ref: v43d regOFF]"
    ...
    if any(c[8] in ("PASS", "S-PASS") for c in comparison):
        ...
        for cfg, avg_sh, min_sh, passes, spasses, fails, avg_t, entry_z, verdict in comparison:
            if passes > 1 or spasses > 1:
                print(
                    "    %s: entry_z=%.1f -> %d PASS + %d S-PASS (potential improvement)"
