#!/usr/bin/env python
"""v65 — Backtest continu 3 ans (2023-04-14 → 2026-04-14).

BASE    : v63 Sweet Spot CERT-03b (entry_z=1.8, exit_z=0.10, blacklist ON)
LEVIER  : Filtres de coïntégration plus stricts pour rejeter les paires spurieuses.

PROBLÈME v63_3y : SLB_PSX, COST_LOW, TFC_V, PNC_V avaient WR=0% → paires
  coïntégrées sur train 2021-2023 mais cassées en 2025 (chocs macro).

MODIFICATIONS vs v63 :
  adf_pvalue_threshold : 0.15 → 0.05   (rejet des fausses coïntégrations)
  min_correlation      : 0.65 → 0.72   (paires plus structurellement liées)
  max_half_life        : 60  → 45 j    (mean-reversion plus rapide = plus fiable)
...
Usage:
    venv\\Scripts\\python.exe scripts/run_v65_3y.py
"""
...
def _apply_settings_v65():
    """v65 — v63 base + filtres coïntégration plus stricts (Levier 1)."""
    s = get_settings()
    ...
    # ── Coïntégration : PLUS STRICT ──────────────────────────────────────
    s.strategy.lookback_window = 180
    s.strategy.additional_lookback_windows = [63]
    s.strategy.adf_pvalue_threshold = 0.05  # v65: 0.05 (était 0.15 en v63)
    s.strategy.min_correlation = 0.72  # v65: 0.72 (était 0.65 en v63)
    if hasattr(s.strategy, "fdr_q_level"):
        s.strategy.fdr_q_level = 0.30
    s.strategy.max_half_life = 45  # v65: 45j (était 60j en v63)
...
def main():
    print("=" * 70)
    print("  v65 — v63 base + filtres coïntégration plus stricts (Levier 1)")
    print("  2023-04-14 -> 2026-04-14 | Capital: 100K$")
    print("  CHANGE  : adf_p=0.05, min_corr=0.72, max_hl=45j")
    ...
    print("=" * 70)
    print("  v65 — RÉSULTATS 3 ANS")
    print("=" * 70)
    print(f"  Sharpe ratio  : {sh:.4f}   (v63_3y: -0.4019)")
    print(f"  Total return  : {ret:+.2f}%   (v63_3y: -3.48%)")
    print(f"  Win rate      : {wr:.1f}%   (v63_3y: 47.1%)")
    print(f"  Total trades  : {t}   (v63_3y: 70)")
    print(f"  Max drawdown  : {dd:.2f}%   (v63_3y: -5.52%)")
    ...
    # ── Save results ────────────────────────────────────────────────────
    results_path = os.path.join(results_dir, "v65_3y_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        f.write("v65 — v63 base + filtres coïntégration plus stricts — 3 ans\n")
        f.write("=" * 60 + "\n")
        f.write("Capital initial : 100 000 $\n")
        f.write("MODIFIE vs v63  : adf_p=0.05, min_corr=0.72, max_hl=45j\n")
        f.write("INCHANGE vs v63 : entry_z=1.8, exit_z=0.10, max_loss=5%\n")
        f.write("  time_stop=45d, blacklist ON(2/30), regime unfav_sz=0.30\n")
        f.write("  momentum=OFF\n")
        f.write(f"\nTrain  : {TRAIN_START} -> {TRAIN_END}\n")
        f.write(f"OOS    : {OOS_START} -> {OOS_END}\n")
        f.write(f"Symbols: {len(SYMBOLS)}\n\n")
        f.write(f"Sharpe ratio  : {sh:.4f}   (v63_3y: -0.4019)\n")
        f.write(f"Total return  : {ret:+.2f}%   (v63_3y: -3.48%)\n")
        f.write(f"Win rate      : {wr:.1f}%   (v63_3y: 47.1%)\n")
        f.write(f"Total trades  : {t}   (v63_3y: 70)\n")
        f.write(f"Max drawdown  : {dd:.2f}%   (v63_3y: -5.52%)\n")
