...
#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""P5 2024H2 standalone — v45b params (same as run_backtest_v45_wf.py).
>>>>>>> origin/main
...
<<<<<<< HEAD
        ...
        print()
        print(f"  P5 2024H2  S={sh:5.2f}  {ret:+6.2f}%  WR={wr:5.1f}%  t={t:2d}  DD={dd:+6.2f}%  [{v}/{elapsed}s]")
=======
        sh  = metrics.sharpe_ratio
        ret = metrics.total_return * 100
        wr  = metrics.win_rate * 100
...
<<<<<<< HEAD
        ...
        print("  " + "-" * 65)
        for lbl, s, vv, r, _w, nt, d in known:
            print(f"    {lbl:<12}  S={s:5.2f}  {r:+6.2f}%  t={nt:2d}  DD={d:+6.2f}%  [{vv}]")
        print(f"    {'P5 2024H2':<12}  S={sh:5.2f}  {ret:+6.2f}%  t={t:2d}  DD={dd:+6.2f}%  [{v}]")
=======
            ("P1 2019H2", -1.57, "FAIL",  -5.60, 0.0, 30, -7.09),
            ("P2 2020H2", -0.66, "FAIL",  -2.03, 0.0, 18, -4.27),
            ("P3 2022H2", +2.21, "PASS", +10.62, 0.0, 33, -2.71),
...
<<<<<<< HEAD
        print(f"  avg={avg:.2f}  PASS={passes_total}/5  -> {overall}")
        print()
