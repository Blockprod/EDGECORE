#!/usr/bin/env python
"""EDGECORE Backtest v26 — Institutional balanced parameters.
...
Institutional stat-arb sizing:
  ...
  - heat: 200% (2x leverage — standard for market-neutral stat-arb)
...
# ── v26 Institutional Balanced Parameters ──
ENTRY_Z = 1.5       # More entry signals (v22 proven)
EXIT_Z = 0.3        # Let winners run to mean (v22 proven)
ALLOC_PCT = 50.0    # 50% per pair — meaningful but not all-in
HEAT = 2.0          # 200% — standard institutional stat-arb leverage
...
def main():
    ...
    print("=" * 70)
    print("  EDGECORE BACKTEST v26 — Institutional Balanced Parameters")
    print("=" * 70)
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("EDGECORE Backtest v26 — Institutional Balanced\n")
        f.write(f"{'=' * 60}\n")
        ...
        f.write(summary)
        f.write("\n\n--- Comparison ---\n")
        f.write("  v22 (2020-25): Return=+62.44% Sharpe=0.85 DD=-12.33%\n")
        f.write("  v25 (v22 8Y):  Return=-24.44% Sharpe=-0.50 DD=-26.52%\n")
        f.write("  v23 (conserv): Return=+2.58%  Sharpe=0.25 DD=-4.42%\n")
        f.write("  v24 (mod-agg): Return=+3.26%  Sharpe=0.17 DD=-9.58%\n")
    print(f"\n[Saved] {out_path}")
