#!/usr/bin/env python
"""EDGECORE v36 -- Phase 3: Fr├®quence & Ex├®cution Algorithmique.
...
def main():
    print("=" * 75)
    print("  EDGECORE v36 -- Phase 3: Fr├®quence & Ex├®cution Algorithmique")
    ...
    metrics = runner.run_unified(
        symbols=V36_SYMBOLS, start_date="2023-03-04", end_date="2026-03-04",
        sector_map=V36_SECTOR_MAP, pair_rediscovery_interval=2,
    ...
    base = {
        "return_pct": 10.17, "sharpe": 1.29, "pf": 4.04,
        "win_rate": 66.7, "trades": 21, "max_dd": -1.98, "calmar": 5.13,
    }

    print("=" * 75)
    print("  v36 RESULTS vs v35 BASELINE")
    print("=" * 75)
    print("  Return:  %+.2f%%  (v35: +%.2f%%)  delta=%+.2f%%" % (
        r['return_pct'], base['return_pct'], r['return_pct'] - base['return_pct']))
    print("  Sharpe:  %.2f   (v35: %.2f)  delta=%+.2f" % (
        r['sharpe'], base['sharpe'], r['sharpe'] - base['sharpe']))
    print("  PF:      %.2f   (v35: %.2f)  delta=%+.2f" % (
        r['pf'], base['pf'], r['pf'] - base['pf']))
    print("  WR:      %.1f%%   (v35: %.1f%%)" % (r['win_rate'], base['win_rate']))
    print("  Trades:  %d     (v35: %d)  delta=%+d" % (
    ...
    results = {
        "version": "v36",
        "phase": "Phase 3 - Fr├®quence & Ex├®cution Algorithmique",
        "modules": [
