#!/usr/bin/env python
"""v60 — Walk-Forward 3-Window — trend_unfavorable_sizing=0.30 (CERT-03b).

KEY CHANGE from v59:
  - NEW param: regime.trend_unfavorable_sizing = 0.30
    → In BULL_TRENDING, shorts are REDUCED to 30% sizing (was hardcoded 0.0 = blocked)
    → In BEAR_TRENDING, longs are REDUCED to 30% sizing (was hardcoded 0.0 = blocked)
  - This lets v55's winning short trades (AMZN_MU +4725, ORCL_MU +2944, MSFT_GOOGL +2902)
    run at 30% sizing instead of being completely blocked by regime.

  All other params identical to v59 (v55 proven base):
...
def _apply_settings_v60():
    """v60 — v59 base + trend_unfavorable_sizing=0.30 (allow reduced shorts in BULL)."""
    s = get_settings()
    ...
    # ── REGIME: THE KEY FIX ─────────────────────────────────────────────
    # v59 had regime ON but BULL_TRENDING hardcoded short_sz=0.0 (blocked all shorts)
    # v55's W2 big winners were shorts: AMZN_MU +4725, ORCL_MU +2944, MSFT_GOOGL +2902
    # NEW: trend_unfavorable_sizing=0.30 → shorts run at 30% sizing in BULL (not blocked)
    s.regime.enabled = True
    s.regime.neutral_sizing = 1.0
    s.regime.trend_favorable_sizing = 1.0
    s.regime.trend_unfavorable_sizing = 0.30  # NEW: was 0.0 (hardcoded block)
...
def main():
    print("=" * 70)
    print("  v60 — Walk-Forward 3-Window — trend_unfavorable_sizing=0.30")
    print("  REGIME ON: neutral=1.0, favorable=1.0, unfavorable=0.30")
    print("=" * 70)
    print()
    ...
    # ── Save results ──────────────────────────────────────────────────────
    results_path = os.path.join(results_dir, "v60_wf3_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        f.write("v60 Walk-Forward 3-Window — trend_unfavorable_sizing=0.30 — CERT-03b\n")
        f.write("=" * 60 + "\n")
        f.write("APPROACH: v59 base + trend_unfavorable_sizing=0.30\n")
        f.write("  regime: ON (neutral=1.0, favorable=1.0, unfavorable=0.30)\n")
        f.write("  In BULL_TRENDING: shorts run at 30% (was blocked 0%)\n")
        f.write("  In BEAR_TRENDING: longs run at 30% (was blocked 0%)\n")
        f.write("  All other params: same as v59 (v55 proven base)\n")
