#!/usr/bin/env python
"""EDGECORE v38 -- Phase 5: Multi-March├® & Scaling.

Adds on top of v37 (Phase 4, 39 symbols):
  5.1  Universe expansion: 39 ÔåÆ 62 US large-cap single stocks only
         No ETFs / no international ÔÇö same statistical regime as v37 core
         Additions are sector-matched to proven v37 pairs:
           ...
           HC +3 (LLY, TMO, ABT)
  5.3  Leverage 1.5├ù (gross exposure = 150% of NAV)
...
# ÔöÇÔöÇ Universe v38 (62 US large-cap single stocks only ÔÇö no ETFs) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
V38_SYMBOLS = [
    # === MARKET BENCHMARK ===
    "SPY",
    ...
    "COST", "TSN",                                    # +2 new (ADM removed ÔÇö small-cap behaviour)
...
def main():
    print("=" * 75)
    print("  EDGECORE v38 -- Phase 5: Multi-March├® & Scaling")
    print("  Universe: %d symbols (v37: 39) ÔÇö US large-cap singles only" % len(V38_SYMBOLS))
    print("  Phase 5: +23 sector-matched US equities + Leverage 1.5├ù")
    print("  No ETFs/intl ÔÇö same statistical regime as v37 proven core")
    ...
    metrics = runner.run_unified(
        ...
        time_stop=time_stop,
        leverage_multiplier=1.5,   # Phase 5.3: 1.5├ù leverage (sector fix applied)
    )
    ...
    # v37+lever=1.5 benchmark (after sector fix ÔÇö same Sharpe, 1.5├ù return)
    ...
    if s >= 2.0 and pf >= 2.5:
        print()
        print("  >>> ALL TARGETS MET ÔÇö Phase 5 VALIDATED <<<")
    else:
        missing = []
        if s < 2.0:
        ...
        print()
        print(f"  >>> Partial targets met ÔÇö check: {', '.join(missing)} <<<")
