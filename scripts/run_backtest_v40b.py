        ...
        with tqdm(total=len(symbols), desc="  [IBKR 1h] Loading", ncols=72) as pbar:
            for sym in symbols:
                try:
                    ...
                    failed.append(sym)
                    tqdm.write("    WARN %s: %s" % (sym, str(exc)[:80]))
                pbar.update(1)
...
def main():
    ...
    print("  Universe: %d symbols | Window: %s -> %s" % (
        len(V40B_SYMBOLS), START_DATE, END_DATE))
    print("  Leverage: 1.5x  Bars/day: %d  Ann: 252x%d=%d bars/year" % (
        BARS_PER_DAY, BARS_PER_DAY, 252 * BARS_PER_DAY))
    print()
    ...
    time_stop = TimeStopManager(TimeStopConfig(
        half_life_multiplier=1.5, max_days_cap=28, default_max_bars=28,
    ))
    ...
    print("  Loaded: %d bars x %d symbols  (~%.0f trading days)  [%.0fs]" % (
        n_bars, n_syms, n_days, elapsed_load))
    print("  Period: %s -> %s" % (
        str(prices_1h.index[0])[:16], str(prices_1h.index[-1])[:16]))
    print()
    ...
    print("  Simulation completed in %.0fs" % elapsed_sim)
    print()
    ...
    print()
    print("  Return     : %+.2f%%" % ret)
    print("  Sharpe     : %.2f   (ann. at 252 x %d = %d bars/year)" % (
    ...
    print("  MaxDD      : %.2f%%" % dd)
    print("  Calmar     : %.2f" % cal)
    print("  Data       : %d bars, %.0f trading days, %.2f years active" % (
        n_bars, active_trading_days, years))
    print()
    print("  PHASE 3 TARGET CHECK:")
    print("    Trades >= 200/year   : %s (~%.0f/yr)" % (
        "PASS" if trades_per_year >= 200 else "MISS", trades_per_year))
    print("    Sharpe >= 1.5        : %s (%.2f)" % (
        "PASS" if sh >= 1.5 else "MISS", sh))
    print("    Sharpe >= 2.0 (str.) : %s (%.2f)" % (
    ...
    print("    Return : %+.2f%%  (v39: +42.55%%)" % ret)
    print("    Sharpe : %.2f   (v39: 1.82)" % sh)
    print("    WR     : %.1f%%  (v39: 65.2%%)" % wr)
    print("    Trades : %.0f/yr  (v39: ~8/yr)" % trades_per_year)
