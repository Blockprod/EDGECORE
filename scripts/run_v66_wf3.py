#!/usr/bin/env python
"""v66 Walk-Forward 3-Window — Validation CERT-03b.

OBJECTIF : Valider v66 (lookback=120, additional=[42]) sur le protocole
  CERT-03b standard avant adoption comme nouveau Sweet Spot.

RAPPEL DES RÉSULTATS CONTINUS 3 ANS :
  v63_3y : Sharpe -0.40, return -3.48%  (old Sweet Spot — cassures 2025)
  v66_3y : Sharpe +0.33, return +2.21%  (lookback court — MEILLEUR)

PARAMÈTRES v66 vs v63 :
  MODIFIE : lookback_window=120 (était 180), additional=[42] (était [63])
...
Usage:
    venv\\Scripts\\python.exe scripts/run_v66_wf3.py
"""
...
# ── Walk-Forward Windows (identiques au CERT-03b v63/v64) ────────────────────
WF_WINDOWS = [
...
def _apply_settings_v66():
    """v66 — v63 base + lookback plus court (Levier 2)."""
    s = get_settings()
...
def main():
    print("=" * 70)
    print("  v66 Walk-Forward 3-Window — Validation CERT-03b")
    print("  lookback=120, additional=[42]")
    print("  entry_z=1.8, exit_z=0.10, max_loss=5%, time_stop=45d")
    print("  blacklist ON(2/30), REGIME ON: unfav_sz=0.30")
    print()
    print("  RÉFÉRENCE v63 WF3 : Combined Sharpe=0.5554  CERT=PASS")
    print("=" * 70)
    print()
    ...
    print("=" * 70)
    print("  CERT-03b AGGREGATE RESULTS — v66")
    print("=" * 70)
    ...
    # ── Save results ──────────────────────────────────────────────────────
    results_path = os.path.join(results_dir, "v66_wf3_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        f.write("v66 Walk-Forward 3-Window — Validation CERT-03b\n")
        f.write("=" * 60 + "\n")
        f.write("MODIFIE vs v63 : lookback=120, additional=[42]\n")
