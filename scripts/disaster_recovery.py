...
class DisasterRecovery:
    ...
    def recover_from_crash(self, skip_broker_check: bool = False) -> dict:
        ...
        """
        logger.critical("DISASTER_RECOVERY_INITIATED", timestamp=datetime.now(UTC))

        report = {"status": "in_progress", "start_time": datetime.now(UTC), "steps": []}
        ...
        try:
            ...
            if not skip_broker_check:
                ...
                report["steps"].append(
                    {"step": 3, "action": "reconstruct_positions", "success": True, "positions": len(positions)}
                )
            else:
                logger.info("recovery_step", step=3, action="skipping_broker_reconciliation")
                report["steps"].append({"step": 3, "action": "skipped (--skip-broker-check)", "success": True})
            ...
            report["steps"].append(
                {"step": 4, "action": "save_report", "success": True, "report_file": str(report_file)}
            )
    ...
    def verify_data_integrity(self, audit_data: list) -> dict:
        ...
        logger.info("data_integrity_check", is_valid=is_valid, issues_count=len(issues), records=len(audit_data))

        return {"is_valid": is_valid, "issues": issues, "records_checked": len(audit_data)}
