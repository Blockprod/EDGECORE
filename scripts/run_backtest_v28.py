#!/usr/bin/env python
"""EDGECORE Backtest v28 ÔÇö Post-correction validation (├ëtapes 1-5).

All 5 structural corrections applied on the same 2023-2026 window as v27:
  ├ëtape 1: Market regime filter (SPY MA50/200 + realized vol) ÔåÆ block TRENDING
  ├ëtape 2: entry_z raised to 2.0 (was 1.5), min spread $0.50
  ├ëtape 3: Dynamic pair blacklist (2 consecutive losses ÔåÆ 30-day cooldown)
  ├ëtape 4: Short sizing ├ù0.5 in TRENDING/NEUTRAL bull
  ├ëtape 5: Time stop multiplier 2.0 (was 3.0)

v27 baseline:  -26.55%, Sharpe -1.28, 35.8% win rate, 5 time stops, GE_RTX ├ù6
...
# ÔöÇÔöÇ Universe ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
# Same 31 symbols as v27 + SPY (required for regime filter ├ëtape 1)
SYMBOLS = [
    "SPY",  # Index ÔÇö required for market regime filter
...
SECTOR_MAP = {
    # SPY excluded from sector map ÔÇö it's the regime reference, not a trading symbol
...
# ÔöÇÔöÇ v28 Parameters (all corrections applied) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
ENTRY_Z = 2.0           # ├ëtape 2: raised from 1.5
EXIT_Z = 0.3
ALLOC_PCT = 90.0
HEAT = 4.0
STOP_PCT = 0.10
...
FDR_Q = 0.20
REDISCOVERY = 3
MIN_SPREAD = 0.50       # ├ëtape 2: min absolute spread filter
SHORT_MULT = 0.50       # ├ëtape 4: short sizing in bull trend
TIME_STOP_MULT = 2.0    # ├ëtape 5: was 3.0
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="EDGECORE Backtest v28 ÔÇö Post-correction validation"
    )
    ...
    # ÔöÇÔöÇ Configure settings (Singleton) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    settings.strategy.entry_z_min_spread = MIN_SPREAD       # ├ëtape 2
    ...
    # ├ëtape 3: Dynamic pair blacklist (enabled by default, confirm)
    ...
    # ├ëtape 5: Time stop multiplier is set via TimeStopConfig default (2.0)
    ...
    # ÔöÇÔöÇ Save results ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    os.makedirs(os.path.join(_ROOT, "results"), exist_ok=True)
    out_path = os.path.join(_ROOT, "results", "bt_results_v28.txt")
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("EDGECORE Backtest v28 ÔÇö Post-Correction Validation\n")
        f.write(f"{'=' * 60}\n")
        ...
        f.write(f"Corrections: Regime=ON, Blacklist=ON, Short├ù={SHORT_MULT}, ")
        f.write(f"TimeStop={TIME_STOP_MULT}├ùHL\n")
        f.write(f"Elapsed: {elapsed:.1f}s\n\n")
        f.write(summary)
    print(f"\n[Saved] {out_path}")

    # ÔöÇÔöÇ v28 success criteria check ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    print("\n" + "=" * 70)
