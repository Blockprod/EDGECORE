#!/usr/bin/env python
"""v56 — Walk-Forward 3-Window — Balanced Mean Reversion (CERT-03b).

Root cause analysis of v55:
  W1=-0.61, W2=+2.04, W3=-3.79 → aggregate Sharpe=-0.78.
  W2 proved the strategy works. Two problems:
    1) Blacklist too aggressive: W3 had 60 blocked pairs → too few candidates.
       Fix: WL_WR 0.30→0.40, WL_PNL -1500→-3000 (only block extreme losers).
    2) W3=2025 bull market: shorts destroyed WR (14.3%). Need short protection.
       Fix: short_sizing_multiplier=0.25 in neutral, disable_shorts_in_bull=True.
    3) adf_pvalue=0.10 too strict with FDR q=0.20 → very few pairs survive.
       Fix: adf=0.15, fdr_q=0.30 to allow more candidates.
    4) spread_corr=0.50 too strict → blocks valid pairs.
       Fix: spread_corr=0.65.
...
# ── Blacklisting thresholds (relaxed vs v55) ──────────────────────────────
WL_WR_THRESHOLD = 0.40   # was 0.30 — only block truly bad pairs
WL_PNL_THRESHOLD = -3000  # was -1500 — tolerate moderate losers
...
def _apply_settings_v56():
    """v56 balanced mean-reversion parameters."""
    s = get_settings()

    # ── Signal quality (same as v55 — proven in W2) ─────────────────────
    s.strategy.entry_z_score = 1.5
    s.strategy.exit_z_score = 0.05
    s.strategy.z_score_stop = 4.0
    s.strategy.entry_z_min_spread = 0.0

    # ── Cointegration quality (slightly relaxed vs v55) ─────────────────
    s.strategy.lookback_window = 180
    s.strategy.additional_lookback_windows = [63]
    s.strategy.adf_pvalue_threshold = 0.15  # was 0.10 — slightly more pairs
    s.strategy.min_correlation = 0.65  # was 0.70 — slightly wider net
    if hasattr(s.strategy, "fdr_q_level"):
        s.strategy.fdr_q_level = 0.30  # was 0.20 — allow more candidates past FDR
    ...
    s.risk.max_concurrent_positions = 12  # was 10 — slightly more capacity

    # ── Regime filter — RE-ENABLED for short protection ─────────────────
    s.regime.enabled = True
    s.strategy.regime_directional_filter = True
    s.strategy.short_sizing_multiplier = 0.25  # was 1.0 — heavy short reduction
    s.strategy.trend_long_sizing = 1.0
    s.strategy.disable_shorts_in_bull_trend = True  # kill shorts in bull trending

    # ── Momentum — disabled ─────────────────────────────────────────────
    s.momentum.enabled = False

    # ── Pair blacklist (relaxed thresholds) ─────────────────────────────
    s.pair_blacklist.enabled = True
    s.pair_blacklist.max_consecutive_losses = 4  # was 3 — more lenient
...
def _build_blacklist_json(
    ...
    bl_path = os.path.join(
        ...
        "results",
        "v56_cross_window_blacklist.json",
    )
...
def _run_window(
    ...
    # v56: max_correlation=0.65 (was 0.50 — too strict, blocked valid pairs)
    ...
    try:
        metrics = runner.run_unified(
            ...
            pair_rediscovery_interval=3,  # was 5 — more pair opportunities
            allocation_per_pair_pct=25.0,  # was 20 — $25k/pair slightly bigger
...
def main():
    print("=" * 70)
    print("  v56 — Walk-Forward 3-Window — Balanced Mean Reversion (CERT-03b)")
    print("  vs v55: relaxed blacklist, short protection, adf=0.15, alloc=25%")
    print("=" * 70)
    print()
    ...
    # ── Save results ──────────────────────────────────────────────────────
    results_path = os.path.join(results_dir, "v56_wf3_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        f.write("v56 Walk-Forward 3-Window — Balanced Mean Reversion — CERT-03b\n")
        f.write("=" * 60 + "\n")
        f.write("Key parameter changes vs v55:\n")
        f.write("  blacklist WR:  0.30 -> 0.40\n")
        f.write("  blacklist PnL: -1500 -> -3000\n")
        f.write("  adf_p:   0.10 -> 0.15\n")
        f.write("  fdr_q:   0.20 -> 0.30\n")
        f.write("  min_corr: 0.70 -> 0.65\n")
        f.write("  spread_corr: 0.50 -> 0.65\n")
        f.write("  regime: disabled -> enabled (short protection)\n")
        f.write("  short_sizing: 1.0 -> 0.25\n")
        f.write("  disable_shorts_in_bull: False -> True\n")
        f.write("  alloc: 20% -> 25%\n")
        f.write("  max_concurrent: 10 -> 12\n")
        f.write("  pair_rediscovery: 5 -> 3\n")
        f.write("  bl max_consecutive_losses: 3 -> 4\n")
