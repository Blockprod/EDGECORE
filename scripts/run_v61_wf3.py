#!/usr/bin/env python
"""v61 — Walk-Forward 3-Window — Wide Neutral Band (CERT-03b).

KEY CHANGES from v60:
  1. regime.neutral_band_pct = 0.05 (was 0.02)
     → NEUTRAL zone is 5x wider: |ma_spread| < 5% = NEUTRAL (full sizing both sides)
     → Only extreme trends (|spread| > 5%) trigger TRENDING protection
     → In v60, 2025 had 26+ regime transitions with 2% band → whipsaw destroyed W3
     → With 5% band, most moderate moves stay NEUTRAL → eliminates whipsaw
     → W2 benefit: shorts in moderate bull (2-5% spread) run at 100% not 30%

  2. regime.trend_unfavorable_sizing = 0.20 (was 0.30)
     → When TRENDING IS triggered (extreme >5% moves), be more conservative
     → Protects W3 in extreme trend episodes while wider band handles the rest

  All other params identical to v60/v59 (v55 proven base):
    entry_z=1.5, exit_z=0.05, z_stop=4.0, max_loss=5%, time_stop=60d
    neutral_sizing=1.0, trend_favorable_sizing=1.0
    regime ON, blacklist OFF, momentum OFF
...
def _apply_settings_v61():
    """v61 — Wide Neutral Band: neutral_band=5%, unfav_sz=0.20."""
    s = get_settings()
    ...
    # ── REGIME: WIDE NEUTRAL BAND ───────────────────────────────────────
    # v60 problem: neutral_band=2% → 26+ regime transitions in W3 → whipsaw
    #   BULL at spread 2-5% forced short_sz=0.30 → losses from bad timing
    #   Rapid BULL→MEAN_REVERTING→BULL cycling in choppy 2025
    #
    # v61 fix: neutral_band=5% → moderate moves (2-5%) stay NEUTRAL
    #   Only extreme trends (>5% spread) trigger TRENDING protection
    #   W2 benefit: shorts in moderate bull run at 100% (not 30%) → closer to v55's +2.04
    #   W3 benefit: far fewer regime transitions → stable sizing → no whipsaw
    #
    # unfav_sz reduced to 0.20 (was 0.30): when TRENDING fires (extreme moves),
    # be more conservative on the unfavorable side for safety.
    s.regime.enabled = True
    s.regime.neutral_sizing = 1.0
    s.regime.trend_favorable_sizing = 1.0
    s.regime.trend_unfavorable_sizing = 0.20  # v61: reduced from 0.30 (extreme trend only)
    s.regime.neutral_band_pct = 0.05          # v61: 5% (was 2%) — wide neutral zone
...
def main():
    print("=" * 70)
    print("  v61 — Walk-Forward 3-Window — Wide Neutral Band")
    print("  REGIME ON: neutral_band=5%, unfav_sz=0.20")
    print("  neutral=1.0, favorable=1.0, unfavorable=0.20")
    print("=" * 70)
    print()
    ...
    # ── Save results ──────────────────────────────────────────────────────
    results_path = os.path.join(results_dir, "v61_wf3_results.txt")
    with open(results_path, "w", encoding="utf-8") as f:
        f.write("v61 Walk-Forward 3-Window — Wide Neutral Band — CERT-03b\n")
        f.write("=" * 60 + "\n")
        f.write("APPROACH: v60 base + neutral_band=5% + unfav_sz=0.20\n")
        f.write("  regime: ON (neutral=1.0, favorable=1.0, unfavorable=0.20)\n")
        f.write("  neutral_band_pct: 0.05 (was 0.02)\n")
        f.write("  Only extreme trends (|spread|>5%) trigger TRENDING\n")
        f.write("  Moderate moves (2-5%) stay NEUTRAL → full sizing both sides\n")
