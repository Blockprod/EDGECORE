...
LESSONS LEARNED:
  ...
  3. But avg_loss($661) >> avg_win($390) — noisy sectors add large-loss pairs
...
v31d APPROACH: "Curated quality universe"
  ...
  - Total: 74 symbols → ~420 intra-sector pairs (vs 806 in v31c)
...
SYMBOLS = [
    "SPY",
    # Technology (Mega Cap) — tight structural relationships
    "AAPL", "MSFT", "GOOGL", "META", "NVDA", "AMD", "INTC", "AVGO",
    "CRM", "ADBE",
    # Technology (Semiconductors) — commodity-cycle driven
    "MRVL", "ON", "MCHP", "QCOM", "TXN", "AMAT", "LRCX", "KLAC",
    # Financials (Mega Cap) — rate-driven, similar exposures
    "JPM", "BAC", "GS", "MS", "WFC", "C", "BLK", "SCHW",
    # Healthcare (Mega Cap Pharma Only) — stable, no biotech noise
    "JNJ", "UNH", "MRK", "ABBV", "LLY", "TMO", "ABT",
    # Consumer Staples — defensive, stable relationships
    "KO", "PEP", "PG", "CL", "WMT", "COST",
    # Energy — commodity-driven, tight correlations
    "XOM", "CVX", "COP", "SLB", "EOG", "VLO", "MPC", "PSX", "DVN",
    "HAL", "BKR",
    # Industrials — cyclical, similar macro exposure
    "CAT", "DE", "HON", "GE", "RTX", "LMT", "MMM", "EMR", "ITW",
    "ROK", "CMI", "PH",
    # Utilities — rate-sensitive, stable pairs
    "NEE", "DUK", "SO", "D",
...
# Regional banks (USB,PNC,TFC,RF,CFG,HBAN,KEY) — post-SVB structural divergence
# Biotech (GILD,REGN,BIIB,VRTX,BMY,ZTS,MCK) — binary events, not mean-reverting
# Healthcare services (CVS,CI,HUM,ELV,CNC) — regulation/M&A driven
# Communication (CMCSA,DIS,NFLX,FOXA,VZ,T) — secular cord-cutting trends
# Consumer discretionary (TGT,LOW,HD,ROST,TJX,DLTR,DG) — post-COVID structural
# REITs (PLD,AMT,SPG) — rate-sensitive, only 3 symbols = trivial FDR
# PFE — patent cliff (COVID vaccine revenue -90%)
...
ALLOC_PCT = 50.0        # v30b proven — full conviction sizing
...
def main():
    ...
    print("  --- v31d Approach ---")
    print("  Curated universe: removed regional banks, biotech, healthcare svc,")
    print("    communication, consumer discretionary, REITs, PFE")
    print("  Kept: tech(20), fin(9), health(7), staples(7), energy(12),")
    print("    industrials(13), utilities(5)")
    print("  Momentum:     STRICT (blocks ALL contra-momentum)")
    print("  All params:   v30b-proven baseline")
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write("Curated: removed noisy sectors (regional banks, biotech, etc.)\n")
