...
Fix: add exactly 1 new symbol per sector ÔÇö sector pools grow by only 1,
     keeping FDR selection stable, preserving proven v37 pairs.

v37 core (39 symbols) + 6 curated additions = 45 total:
  Tech     +1: QCOM  (semiconductor ÔÇö confirmed AMD_QCOM pair in testing)
  Fin      +1: BLK   (asset mgmt ÔÇö distinct niche within financials)
  Energy   +1: SLB   (oilfield services ÔÇö different micro from E&P)
  CS       +1: COST  (warehouse retail ÔÇö different model from WMT/KO/PEP)
  Indust   +1: LMT   (defense ÔÇö distinct from HON/CAT/DE industrials)
  HC       +1: LLY   (pharma mega-cap ÔÇö confirmed high-volume)
...
# ÔöÇÔöÇ Universe v38d: v37 core + 1 curated symbol per sector ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
V38D_SYMBOLS = [
    # === MARKET BENCHMARK ===
    "SPY",
    ...
    "QCOM",   # +1: semiconductor ÔÇö pairs with AMD/NVDA

    # === FINANCIALS (7 v37 + 1 new = 8) ===
    "JPM", "GS", "BAC", "MS", "WFC", "C", "SCHW",
    "BLK",    # +1: asset management ÔÇö distinct niche

    # === ENERGY (4 v37 + 1 new = 5) ===
    "XOM", "CVX", "COP", "EOG",
    "SLB",    # +1: oilfield services ÔÇö pairs with E&P names
...
def main():
    ...
    print(f"  Universe: {n} symbols (v37: 39, +6 curated) ÔÇö US large-cap singles")
    print("  Phase 5.3: Leverage 1.5├ù with sector-weight fix")
    print("  v37+L1.5 baseline: +21.42%  S1.67  PF8.41  WR65.2%  23t  DD-1.69%")
    print("=" * 75)
    print()

    s = get_settings()
    # ÔöÇÔöÇ Exact v37 settings ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    # ÔöÇÔöÇ Results vs baselines ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    if s >= 1.5 and t > v37_t:
        ...
        print(f"  >>> Sharpe target MET ÔÇö trades at {t} (need > {v37_t}) <<<")
    else:
        print(f"  >>> Partial targets met ÔÇö check: Sharpe {s:.2f}<1.5, trades {t} <<<")
    print()
