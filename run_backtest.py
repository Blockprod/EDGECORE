...
if __name__ == "__main__":
    ...
    def main():
        ...
        parser.add_argument("--start", type=str, default="2020-01-01", help="Date de début du backtest")
        ...
        parser.add_argument("--rediscovery", type=int, default=3, help="Intervalle de redécouverte des paires (bars)")
        ...
        try:
            with open("debug_symbols_snapshot.txt", "w", encoding="utf-8") as df:
        ...
        if problematic:
            print(f"[WARNING] {len(problematic)} problematic tickers rejected. See debug_symbols_snapshot.txt for details.")
      ...
      weekly_confirmation=args.weekly_confirmation,
        ...
        with open("bt_results_summary.txt", "w", encoding="utf-8") as f:
          f.write(summary)
        # Sauvegarde des métriques en JSON
        metrics_dict = metrics.__dict__ if hasattr(metrics, "__dict__") else dict(metrics)
        with open("bt_results_metrics.json", "w", encoding="utf-8") as f:
          json.dump(metrics_dict, f, indent=2, ensure_ascii=False)
        # Sauvegarde des retours quotidiens en CSV
        if hasattr(metrics, "returns"):
          returns = metrics.returns if isinstance(metrics.returns, pd.Series) else pd.Series(metrics.returns)
          returns.to_csv("bt_results_returns.csv", index=True)
