...
class OHLCVValidator:
    ...
    def validate(
        self, df: pd.DataFrame, raise_on_error: bool = False, max_age_hours: float = 99999.0
    ) -> ValidationResult:
        """
...
>>>>>>> origin/main
            ...
            if time_delta.total_seconds() < 300:  # 5 minutes
                if equity_change_pct > self.max_jump_pct:
                    warnings.append(f"Equity jump {equity_change_pct:.2f}% in {time_delta.total_seconds():.0f}s")
