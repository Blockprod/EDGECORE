...
from common.types import CommissionType, SlippageModel
...
def test_t1_2():
    ...
    slippage_calc = SlippageCalculator({"model": SlippageModel.FIXED_BPS, "fixed_bps": 5.0})

    # Test buy order slippage
    slippage_bps, exec_price = slippage_calc.calculate(
        order_price=100.0, market_price=100.0, order_quantity=1.0, market_volume=1000.0, side="buy"
    )
    assert slippage_bps == 5.0
    assert exec_price > 100.0  # Buy prices go UP with slippage
    print(f"[OK] Buy slippage: 100.00  {exec_price:.2f} (+{slippage_bps}bps)")

    # Test sell order slippage
    slippage_bps, exec_price = slippage_calc.calculate(
        order_price=100.0, market_price=100.0, order_quantity=1.0, market_volume=1000.0, side="sell"
    )
    assert slippage_bps == 5.0
    assert exec_price < 100.0  # Sell prices go DOWN with slippage
    print(f"[OK] Sell slippage: 100.00  {exec_price:.2f} (-{slippage_bps}bps)")
    ...
    commission_calc = CommissionCalculator({"type": CommissionType.PERCENT, "percent": 0.1})
