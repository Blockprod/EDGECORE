...
class TestParallelPairDiscovery:
    ...
    def test_parallel_discovery_faster_than_sequential(self):
        ...
        data = pd.DataFrame({sym: np.random.randn(100).cumsum() for sym in symbols})
...
>>>>>>> origin/main
    ...
    def test_parallel_with_num_workers(self):
        ...
        np.random.seed(42)
        data = pd.DataFrame({f"SYM{i}": np.random.randn(100).cumsum() for i in range(5)})

        strategy = PairTradingStrategy()

        # Test with 2 workers
        pairs = strategy.find_cointegrated_pairs_parallel(data, num_workers=2)
...
class TestCachingFunctionality:
    ...
    def test_find_cointegrated_pairs_uses_cache(self):
        ...
        np.random.seed(42)
        data = pd.DataFrame({f"SYM{i}": np.random.randn(100).cumsum() for i in range(4)})
...
class TestPerformanceMetrics:
    ...
    def test_parallel_pair_discovery_metrics(self):
        ...
        np.random.seed(42)
        data = pd.DataFrame({f"SYM{i}": np.random.randn(100).cumsum() for i in range(6)})
    ...
    def test_cache_eliminates_redundant_discovery(self):
        ...
        np.random.seed(42)
        data = pd.DataFrame({f"SYM{i}": np.random.randn(50).cumsum() for i in range(4)})
...
>>>>>>> origin/main
    ...
    def test_force_non_parallel_discovery(self):
        ...
        # Force sequential
        pairs = strategy.find_cointegrated_pairs(data, use_cache=False, use_parallel=False)
