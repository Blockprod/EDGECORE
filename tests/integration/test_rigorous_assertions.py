...
class TestFullPipelineNoLeakage:
    ...
    def test_walk_forward_splits_no_overlap(self):
        ...
        data = pd.DataFrame(
            {"AAPL": np.random.randn(365).cumsum() + 100, "MSFT": np.random.randn(365).cumsum() + 50}, index=dates
        )
    ...
    def test_simulator_completes_with_real_metrics(self):
        ...
        assert metrics.max_drawdown <= 0.0, f"max_drawdown should be <= 0, got {metrics.max_drawdown}"
...
class TestBacktestMatchesLiveSignals:
    ...
    def test_signal_determinism(self):
        ...
        # Same z-scores
        pd.testing.assert_series_equal(info1["z_score"], info2["z_score"], check_names=False)
    ...
    def test_strategy_generate_signals_deterministic(self):
        ...
        assert len(signals1) == len(signals2), f"Non-deterministic signal count: {len(signals1)} vs {len(signals2)}"
...
class TestDynamicSpreadModelExactSignals:
    ...
    def test_extreme_negative_spread_produces_long(self):
        ...
        signals, _info = model.get_adaptive_signals(forced)
        assert signals.iloc[-1] == 1, f"Expected long (1) for extreme negative spread, got {signals.iloc[-1]}"
    ...
    def test_extreme_positive_spread_produces_short(self):
        ...
        signals, _info = model.get_adaptive_signals(forced)
        assert signals.iloc[-1] == -1, f"Expected short (-1) for extreme positive spread, got {signals.iloc[-1]}"
    ...
    def test_mean_spread_produces_hold(self):
        ...
        signals, _info = model.get_adaptive_signals(forced)
        assert signals.iloc[-1] == 0, f"Expected hold (0) for spread at mean, got {signals.iloc[-1]}"
...
class TestMetricsComputedValues:
    ...
    def test_positive_returns_produce_positive_total_return(self):
        ...
        trades: list[float] = [100.0, 200.0, 150.0]

        metrics = BacktestMetrics.from_returns(
            returns=returns, trades=trades, start_date="2023-01-01", end_date="2023-01-05"
        )
    ...
    def test_all_losing_trades_zero_win_rate(self):
        ...
        trades: list[float] = [-100.0, -50.0, -200.0]

        metrics = BacktestMetrics.from_returns(
            returns=returns, trades=trades, start_date="2023-01-01", end_date="2023-01-03"
        )

        assert metrics.win_rate == 0.0, f"All losing trades should have win_rate=0, got {metrics.win_rate}"
