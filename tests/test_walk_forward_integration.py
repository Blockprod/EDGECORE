...
class TestSplitWalkForward:
    ...
    def test_split_creates_correct_period_count(self):
        ...
        data = pd.DataFrame(
            {"BTC/USDT": np.random.randn(252).cumsum() + 100, "ETH/USDT": np.random.randn(252).cumsum() + 50},
            index=dates,
        )
    ...
    def test_split_creates_train_larger_than_test(self):
        ...
        dates = pd.date_range("2024-01-01", periods=252, freq="D")
        data = pd.DataFrame({"BTC/USDT": np.random.randn(252).cumsum() + 100}, index=dates)
...
class TestWalkForwardBacktest:
    ...
    def test_walk_forward_per_period_breakdown(self):
        ...
        result = backtester.run_walk_forward(
            symbols=["BTC/USDT"], start_date="2023-01-01", end_date="2023-12-31", num_periods=2, use_synthetic=True
        )
...
class TestWalkForwardPrintSummary:
    ...
    def test_print_summary_returns_string(self):
        ...
        _result = backtester.run_walk_forward(
            symbols=["BTC/USDT"], start_date="2023-01-01", end_date="2023-12-31", num_periods=2, use_synthetic=True
        )
