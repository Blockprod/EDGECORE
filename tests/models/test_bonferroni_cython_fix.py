...
class TestBonferroniAlphaCorrection:
    ...
    def test_alpha_corrected_10_symbols(self):
        """10 symbols Ôåô 45 pairs Ôåô ╬▒ = 0.05/45 Ôëê 0.00111."""
        y, x = _make_cointegrated_pair()
        result = engle_granger_test_cpp_optimized(y, x, num_symbols=10, apply_bonferroni=True)
        expected_alpha = 0.05 / 45
        assert abs(result["alpha_threshold"] - expected_alpha) < 1e-10

    def test_no_bonferroni_uses_nominal_alpha(self):
        """apply_bonferroni=False Ôåô ╬▒ = 0.05."""
        y, x = _make_cointegrated_pair()
        result = engle_granger_test_cpp_optimized(y, x, num_symbols=50, apply_bonferroni=False)
        assert result["alpha_threshold"] == 0.05

    def test_no_num_symbols_uses_nominal_alpha(self):
        """num_symbols=None Ôåô ╬▒ = 0.05 (backward compatible)."""
        y, x = _make_cointegrated_pair()
        result = engle_granger_test_cpp_optimized(y, x)
        assert result["alpha_threshold"] == 0.05

    def test_single_symbol_pair_edge_case(self):
        """num_symbols=2 Ôåô 1 pair Ôåô ╬▒ = 0.05 (no correction needed)."""
        y, x = _make_cointegrated_pair()
...
class TestBonferroniChangesDecision:
    ...
    def test_marginal_pair_rejected_with_bonferroni(self):
        ...
        for seed in range(100):
            ...
            r_no_bonf = engle_granger_test_cpp_optimized(y, x, apply_bonferroni=False)
        ...
        assert found, "Could not find a seed producing a marginal p-value between 0.05 and Bonferroni-corrected alpha"

    def test_strong_pair_survives_bonferroni(self):
        """A strongly cointegrated pair (p Ôëê 0) survives Bonferroni."""
...
class TestFalsePositiveReduction:
    ...
    def test_bonferroni_reduces_false_positives(self):
        ...
        for _i in range(n_tests):
            ...
            r_uncorr = engle_granger_test_cpp_optimized(y, x, apply_bonferroni=False)
            r_corr = engle_granger_test_cpp_optimized(y, x, num_symbols=20, apply_bonferroni=True)
