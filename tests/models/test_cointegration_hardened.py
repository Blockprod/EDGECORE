...
=======
        ...
        # With 50 symbols: ~1,225 pairs
        # Corrected alpha: 0.05 / 1225 ≈ 0.0000408
        assert alpha_corrected < alpha_uncorrected
        assert 0.00003 < alpha_corrected < 0.00005
...
<<<<<<< HEAD
    ...
    def create_cointegrated_pair(self, hedge_ratio=1.5, periods=500, noise_std=0.5):
        ...
        # Create price series
        x = pd.Series(100 + np.cumsum(np.random.normal(0, 1, periods)))

        # y cointegrated with x: y ≈ hedge_ratio * x + noise
        y = pd.Series(hedge_ratio * np.asarray(x, dtype=float) + np.random.normal(0, noise_std, periods))
...
<<<<<<< HEAD
        ...
        if (
            hedge1 is not None
            and hedge2 is not None
            and isinstance(hedge1, (int, float))
            and isinstance(hedge2, (int, float))
        ):
    ...
    def test_hedge_ratio_tracker_stability(self):
        ...
        _beta_to_use, is_stable = tracker.reestimate_if_needed(
            pair_key=pair_key, new_beta=1.52, drift_tolerance_pct=10.0
        )
        ...
        _beta_to_use, is_stable = tracker.reestimate_if_needed(
            pair_key=pair_key, new_beta=1.7, drift_tolerance_pct=10.0
=======
...
<<<<<<< HEAD
    ...
    def test_oos_validation_passes_true_pair(self):
        ...
        x_arr = 100 + np.cumsum(np.random.normal(0, 1, 300))
        y_arr = 1.5 * x_arr + np.random.normal(0, 1, 300)

        # Split: 250 train, 50 OOS
        x_train = pd.Series(x_arr[:250])
        x_oos = pd.Series(x_arr[250:])
        y_train = pd.Series(y_arr[:250])
        y_oos = pd.Series(y_arr[250:])
...
<<<<<<< HEAD
    ...
    def test_oos_validation_rejects_false_pair(self):
        ...
        x_arr = 100 + np.cumsum(np.random.normal(0, 1, 300))
        y_arr = 100 + np.cumsum(np.random.normal(0, 1, 300))
    ...
    def test_oos_consistency_threshold(self):
        ...
        # Generate cointegrated pair
        x_arr = 100 + np.cumsum(np.random.normal(0, 1, 500))
        y_arr = 1.5 * x_arr + np.random.normal(0, 1, 500)
        ...
        for i in range(5):
            ...
            if end <= 500:
                x_window = pd.Series(x_arr[start:end])
                y_window = pd.Series(y_arr[start:end])
                total_days += 1
...
=======
    ...
    def test_oos_consistency_threshold(self):
        ...
        # Expect reasonable consistency for true pair (≥0% due to random variation)
>>>>>>> origin/main
...
<<<<<<< HEAD
        ...
        # AR(1) regression
        X = np.asarray(centered[:-1], dtype=float).reshape(-1, 1)
        y = np.asarray(centered[1:], dtype=float)
...
=======
        ...
        # For random walk (ρ ≈ 1), may return None or very high HL (>50)
        # Due to AR(1) estimation, rho might be ~0.98-0.99, giving HL > 50
        assert estimated_hl is None or estimated_hl > 50  # Non-stationary indicator
...
<<<<<<< HEAD
    ...
    def test_identical_series_cointegration(self):
        """Test identical series (ρ=1) is trivial case."""
...
<<<<<<< HEAD
    ...
    def test_spread_computation_reproducibility(self):
        ...
        np.testing.assert_array_almost_equal(
            np.asarray(spread1, dtype=float),
            np.asarray(spread2, dtype=float),
        )
    ...
    def test_spread_is_stationary(self):
        ...
        var_x = np.var(np.asarray(x, dtype=float))
        var_y = np.var(np.asarray(y, dtype=float))
        spread_vals = np.asarray(spread, dtype=float)
