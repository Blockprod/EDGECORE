...
class TestAdaptiveThresholdCalculator:
    ...
    def test_volatility_adjustment_low_regime(self):
        ...
        calc = AdaptiveThresholdCalculator(
            ThresholdConfig(volatility_adjustment_enabled=True)
        )
    ...
    def test_half_life_adjustment_short(self):
        ...
        calc = AdaptiveThresholdCalculator(
            ThresholdConfig(hl_adjustment_enabled=True)
        )
        
        spread = pd.Series(np.random.randn(100))
        entry, exit_t, details = calc.calculate_threshold(
            spread,
    ...
    def test_position_sizing(self):
        ...
        # Normal case
        size = calc.calculate_position_sizing(
            portfolio_vol=0.15,
            spread_vol=0.05,
        ...
        size_high_vol = calc.calculate_position_sizing(
            portfolio_vol=0.15,
            spread_vol=0.20,
            target_risk_pct=0.01
        )
...
class TestThresholdAdaptationScenarios:
    ...
    def test_transition_regime_scenario(self):
        ...
        entry_calm, _, details_calm = calc.calculate_threshold(
            volatility_profile[:120],
            half_life=20.0
        )
        
        # Threshold on volatile part
        entry_volatile, _, details_volatile = calc.calculate_threshold(
            volatility_profile[100:200],
            half_life=20.0
        )
