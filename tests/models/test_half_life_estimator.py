...
class TestOUProcessHalfLife:
    ...
    def generate_ou_process(
        self, half_life: float, periods: int = 500, noise_std: float = 0.1, seed: int = 42
    ) -> pd.Series:
...
class TestLookAheadFree:
    """P3-01 — Verify the EWM centering is strictly causal (no look-ahead bias)."""

    def _gen_ou(self, half_life: float, n: int = 600, seed: int = 7) -> pd.Series:
        np.random.seed(seed)
        mr = np.log(2) / half_life
        ou = np.zeros(n)
        for t in range(1, n):
            ou[t] = ou[t - 1] - mr * ou[t - 1] + np.random.normal(0, 0.2)
        return pd.Series(ou)

    def test_estimator_uses_causal_ewm(self):
        """EWM mean must be based on data.shift(1) — no look-ahead into current bar."""
        import inspect

        from models import half_life_estimator as mod

        src = inspect.getsource(mod.SpreadHalfLifeEstimator.estimate_half_life_from_spread)
        # The fix: shift(1) before ewm so the mean at bar t sees only bars 0..t-1
        assert "shift(1)" in src, "Look-ahead bias fix missing: EWM mean must be computed on data.shift(1)"
        assert "adjust=False" in src, "Look-ahead bias fix missing: EWM must use adjust=False for causal recursion"

    def test_causal_estimator_not_shorter_than_biased(self):
        """Causal EWM (shift=1) must not give systematically shorter HL than biased EWM.

        The biased version (no shift) includes the current bar in the demeaning EWM,
        making the centered series appear more mean-reverting → shorter HL.
        The corrected version (shift=1) is strictly causal and should produce equal
        or longer HL estimates on average.
        """
        true_hl = 40
        causal_hls = []
        biased_hls = []
        ...
        for seed in range(10):
            spread = self._gen_ou(true_hl, n=600, seed=seed)
            lookback = 252
            data = spread.iloc[-lookback:].copy()
            ewm_span = max(int(lookback * 2 / 3), 20)

            # Biased: EWM uses current value
            ewm_biased = data.ewm(span=ewm_span, adjust=True).mean()
            centered_biased = data - ewm_biased
            lag_b = centered_biased.shift(1)
            mask_b = lag_b.notna()
            X_b = np.asarray(lag_b[mask_b], dtype=float).reshape(-1, 1)
            y_b = np.asarray(centered_biased[mask_b], dtype=float)
            if len(X_b) >= 10:
                rho_b = np.linalg.lstsq(X_b, y_b, rcond=None)[0][0]
                if 0 < rho_b < 1:
                    biased_hls.append(-np.log(2) / np.log(rho_b))

            # Causal: shift(1) before EWM
            ewm_causal = data.shift(1).ewm(span=ewm_span, adjust=False).mean()
            centered_causal = data - ewm_causal
            lag_c = centered_causal.shift(1)
            mask_c = lag_c.notna()
            X_c = np.asarray(lag_c[mask_c], dtype=float).reshape(-1, 1)
            y_c = np.asarray(centered_causal[mask_c], dtype=float)
            if len(X_c) >= 10:
                rho_c = np.linalg.lstsq(X_c, y_c, rcond=None)[0][0]
                if 0 < rho_c < 1:
                    causal_hls.append(-np.log(2) / np.log(rho_c))

        assert len(causal_hls) >= 5, f"Too few valid causal estimates: {len(causal_hls)}"
        assert len(biased_hls) >= 5, f"Too few valid biased estimates: {len(biased_hls)}"

        median_causal = float(np.median(causal_hls))
        median_biased = float(np.median(biased_hls))
        # Causal version should be >= biased version (less optimistic)
        assert median_causal >= median_biased * 0.85, (
            f"Causal HL median ({median_causal:.1f}) is shorter than biased ({median_biased:.1f}) "
            "— look-ahead fix may be incorrect"
        )
