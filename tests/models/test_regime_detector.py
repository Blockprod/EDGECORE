...
class TestRegimeClassification:
    ...
    def test_high_volatility_regime(self):
        ...
        # Then feed in high volatility – should be HIGH relative to earlier
        base = 100.0
        for i in range(10):
    ...
    def test_normal_volatility_regime(self):
        ...
        # Tier 1 (low): constant ↓ tiny vol
        for i in range(7):
            detector.update(spread=100.0)
        
        # Tier 2 (high): big swings ↓ high vol
        for i in range(7):
            detector.update(spread=100.0 + (10.0 if i % 2 == 0 else -10.0))
        
        # Tier 3 (mid): moderate swings ↓ falls between 33rd and 67th percentile
        for i in range(6):
...
class TestRegimeTransitions:
    ...
    def test_regime_transition_tracking(self):
        ...
        # Sprint 3.4: Assert at least one transition was recorded (low↓high)
        assert len(detector.regime_transitions) >= 1, (
            f"Expected at least 1 regime transition (low↓high), got {len(detector.regime_transitions)}"
        )
...
class TestRealisticScenarios:
    ...
    def test_calm_market_then_crisis(self):
        ...
        assert calm_regime == VolatilityRegime.LOW, (
            f"Expected LOW regime after calm phase, got {calm_regime}"
        )
    ...
    def test_multiple_regimes_during_trading_day(self):
        ...
        assert stats["regime_transitions"] >= 1, (
            "Expected at least 1 transition (calm↓volatile↓calm)"
        )
