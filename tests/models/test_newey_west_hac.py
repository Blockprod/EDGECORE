...
class TestRobustEdgeCases:
    ...
    def test_numpy_array_input(self):
        ...
        y, x = _cointegrated_pair()
        result = engle_granger_test_robust(
            pd.Series(y.values), pd.Series(x.values), apply_bonferroni=False
        )
        assert result['error'] is None
...
class TestNeweyWestConsensus:
    ...
    def test_bonferroni_forwarded(self):
        ...
        y, x = _cointegrated_pair()
        with mock.patch('models.cointegration.engle_granger_test') as m_std, \
...
class TestPairTradingHACIntegration:
    ...
    def test_static_method_rejects_on_divergence(self):
        ...
        with mock.patch('strategies.pair_trading.engle_granger_test', return_value=mock_eg):
            with mock.patch('strategies.pair_trading._newey_west_consensus', return_value=mock_cons):
    ...
    def test_static_method_accepts_on_consensus(self):
        ...
        with mock.patch('strategies.pair_trading.engle_granger_test', return_value=mock_eg):
            with mock.patch('strategies.pair_trading._newey_west_consensus', return_value=mock_cons):
                with mock.patch('strategies.pair_trading.half_life_mean_reversion', return_value=10):
                    result = PairTradingStrategy._test_pair_cointegration(
                        ('A', 'B', y, x, 0.0, 60, 10, False, True)
                    )
    ...
    def test_nw_disabled_skips_check(self):
        ...
        with mock.patch('strategies.pair_trading.engle_granger_test', return_value=mock_eg):
            with mock.patch('strategies.pair_trading._newey_west_consensus') as mock_nw:
                with mock.patch('strategies.pair_trading.half_life_mean_reversion', return_value=10):
                    result = PairTradingStrategy._test_pair_cointegration(
