...
class TestRiskConfigSchema:
    ...
    def test_custom_risk_config(self):
        ...
        config = RiskConfigSchema(max_position_size=0.15, max_portfolio_heat=0.25, max_loss_per_trade=0.03)
    ...
    def test_loss_greater_than_heat(self):
        ...
        with pytest.raises(ValidationError):
            RiskConfigSchema(max_loss_per_trade=0.5, max_portfolio_heat=0.3)
...
class TestStrategyConfigSchema:
    ...
    def test_custom_strategy_config(self):
        ...
        config = StrategyConfigSchema(min_spread_bps=10.0, max_spread_bps=100.0, fast_sma_period=30, slow_sma_period=60)
    ...
    def test_invalid_spread_reversal(self):
        ...
        with pytest.raises(ValidationError):
            StrategyConfigSchema(min_spread_bps=50.0, max_spread_bps=30.0)

    def test_invalid_sma_periods_equal(self):
        """Test rejection when fast_sma == slow_sma."""
        with pytest.raises(ValidationError):
            StrategyConfigSchema(fast_sma_period=50, slow_sma_period=50)

    def test_invalid_sma_periods_reversed(self):
        """Test rejection when fast_sma > slow_sma."""
        with pytest.raises(ValidationError):
            StrategyConfigSchema(fast_sma_period=60, slow_sma_period=30)
    ...
    def test_invalid_threshold_reversed(self):
        ...
        with pytest.raises(ValidationError):
            StrategyConfigSchema(entry_threshold_std=1.0, exit_threshold_std=2.0)
    ...
    def test_invalid_stop_loss_greater_than_profit(self):
        ...
        with pytest.raises(ValidationError):
            StrategyConfigSchema(stop_loss_pct=2.5, take_profit_pct=2.0)
...
class TestExecutionConfigSchema:
    ...
    def test_custom_execution_config(self):
        ...
        config = ExecutionConfigSchema(
            mode=ExecutionMode.LIVE, order_type=OrderType.MARKET, limit_order_timeout_seconds=60
        )
...
class TestDataSourceConfigSchema:
    ...
    def test_custom_data_source_config(self):
        ...
        config = DataSourceConfigSchema(feed_type="websocket", ohlcv_interval_minutes=15, max_lookback_candles=1000)
    ...
    def test_invalid_buffer_too_small(self):
        ...
        with pytest.raises(ValidationError):
            DataSourceConfigSchema(max_lookback_candles=1000, buffer_size=500)
...
class TestAlerterConfigSchema:
    ...
    def test_custom_alerter_config(self):
        ...
        config = AlerterConfigSchema(
            alert_on_position_opened=False, alert_deduplication_seconds=120, max_alerts_per_minute=30
        )
...
class TestBacktestConfigSchema:
    ...
    def test_valid_backtest_config(self):
        ...
        config = BacktestConfigSchema(start_date="2024-01-01", end_date="2024-12-31", initial_equity=10000.0)
    ...
    def test_invalid_equity_zero(self):
        ...
        with pytest.raises(ValidationError):
            BacktestConfigSchema(start_date="2024-01-01", end_date="2024-12-31", initial_equity=0.0)

    def test_invalid_slippage_negative(self):
        """Test rejection of negative slippage."""
        with pytest.raises(ValidationError):
            BacktestConfigSchema(start_date="2024-01-01", end_date="2024-12-31", slippage_pct=-0.1)

    def test_invalid_slippage_too_large(self):
        """Test rejection of slippage > 10%."""
        with pytest.raises(ValidationError):
            BacktestConfigSchema(start_date="2024-01-01", end_date="2024-12-31", slippage_pct=15.0)

    def test_invalid_commission_too_large(self):
        """Test rejection of commission > 10%."""
        with pytest.raises(ValidationError):
            BacktestConfigSchema(start_date="2024-01-01", end_date="2024-12-31", commission_pct=15.0)

    def test_invalid_high_total_costs(self):
        """Test warning for high total costs."""
        with pytest.raises(ValidationError):
            BacktestConfigSchema(start_date="2024-01-01", end_date="2024-12-31", slippage_pct=3.0, commission_pct=3.0)
...
class TestFullConfigSchema:
    ...
    def test_live_trading_valid_limits(self):
        ...
        config_dict = {"execution": {"mode": "live"}, "risk": {"max_position_size": 0.15, "max_portfolio_heat": 0.2}}
    ...
    def test_backtest_mode_with_config(self):
        ...
        config_dict = {
            "execution": {"mode": "backtest"},
            "backtest": {"start_date": "2024-01-01", "end_date": "2024-12-31", "initial_equity": 10000.0},
        }
...
class TestConfigValidationFunction:
    ...
    def test_validate_valid_config(self):
        ...
        config_dict = {"execution": {"mode": "paper"}, "risk": {"max_position_size": 0.1}}
