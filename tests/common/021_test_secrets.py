...
class TestSecretsVault:
    ...
    def test_rotate_secret(self):
        ...
        assert vault.get_metadata("MY_KEY").rotated_at is not None
    ...
    def test_metadata_access_tracking(self):
        ...
        assert vault.get_metadata("MY_KEY").access_count == 0
        
        vault.get_secret("MY_KEY")
        assert vault.get_metadata("MY_KEY").access_count == 1
        
        vault.get_secret("MY_KEY")
        assert vault.get_metadata("MY_KEY").access_count == 2
...
class TestLoadFromEnvironment:
    ...
    def test_load_with_prefix(self):
        ...
        with patch.dict(os.environ, {
            'TRADING_API_KEY': 'key1',
            'TRADING_PASSWORD': 'pass1',
            'OTHER_API_KEY': 'key2'
        }):
            vault = SecretsVault(auto_load_env=False)
...
class TestSecretsIntegration:
    ...
    def test_full_secret_lifecycle(self):
        ...
        assert vault.get_metadata("API_KEY").access_count == 1
        
        # Access again
        vault.get_secret("API_KEY")
        assert vault.get_metadata("API_KEY").access_count == 2
        
        # Rotate
        vault.rotate_secret("API_KEY", "new_value")
        assert vault.get_secret("API_KEY") == "new_value"
        assert vault.get_metadata("API_KEY").access_count == 3  # Incremented by get_secret
        assert vault.get_metadata("API_KEY").rotated_at is not None
