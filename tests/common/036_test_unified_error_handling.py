...
class TestErrorTaxonomy:
    ...
    def test_trading_error_base_class(self):
        """Ô£ô TradingError stores message, category, original error."""
        original = ValueError("original error")
        error = TradingError("Something went wrong", ErrorCategory.RETRYABLE, original)
    ...
    def test_data_error_defaults_transient(self):
        """Ô£ô DataError defaults to TRANSIENT category."""
...
class TestErrorHandler:
    ...
    def test_handle_transient_error_logs_warning(self, mock_logger):
        """Ô£ô TRANSIENT errors logged as WARNING."""
    ...
    def test_handle_retryable_error_logs_error(self, mock_logger):
        """Ô£ô RETRYABLE errors logged as ERROR."""
    ...
    def test_handle_non_retryable_error_logs_critical(self, mock_logger):
        """Ô£ô NON_RETRYABLE errors logged as CRITICAL."""
    ...
    def test_handle_fatal_error_raises(self, mock_logger):
        ...
        assert mock_logger is not None  # Logger is patched — verify it's active
...
class TestErrorHandlingDecorator:
    ...
    def test_decorator_succeeds_first_attempt(self):
        """Ô£ô Successful function doesn't retry."""
        call_count = 0
    ...
    def test_decorator_retries_transient_errors(self):
        """Ô£ô TRANSIENT errors trigger retries."""
        call_count = 0
    ...
    def test_decorator_stops_on_fatal_errors(self):
        """Ô£ô FATAL errors are not retried."""
        call_count = 0
    ...
    def test_decorator_stops_on_non_retryable_errors(self):
        """Ô£ô NON_RETRYABLE errors stop immediately."""
        call_count = 0
    ...
    def test_decorator_respects_max_retries(self):
        """Ô£ô Stops after max_retries attempts."""
        call_count = 0

        @with_error_handling(category=ErrorCategory.TRANSIENT, max_retries=2, backoff_base=0.01)
        def always_fails():
            nonlocal call_count
            call_count += 1
            raise TimeoutError("timeout")
    ...
    def test_decorator_logs_retries(self, mock_logger):
        """Ô£ô Retries are logged with backoff info."""

        @with_error_handling(max_retries=2, backoff_base=0.01, context_prefix="test_")
        def retry_func():
            raise TimeoutError("timeout")
        ...
        assert any("RETRY_AFTER_BACKOFF" in str(call) for call in mock_logger.info.call_args_list)

    def test_decorator_with_function_args(self):
        """Ô£ô Decorator preserves function arguments."""
...
class TestDataLoadingErrorHandling:
    ...
    def test_data_error_raised_on_empty_data(self):
        """Ô£ô Empty data raises DataError with appropriate category."""
