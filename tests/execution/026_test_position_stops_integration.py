...
class TestExecutionIntegrationWithStops:
    ...
    def test_price_updates_trigger_stops(self):
        ...
        assert reason is not None and "stop" in reason.lower()
    ...
    def test_trailing_stop_in_execution_flow(self):
        ...
        assert pos_status.get("stop_loss_price") is not None

        # Price drops below trailing stop
        should_exit, _reason = manager.check_exits("trail_pos", 52000.0)
        assert should_exit is True
    ...
    def test_stop_manager_persistence_across_updates(self):
        ...
        for price in [51000, 52000, 53000, 54000, 55000]:
            ...
            assert status.get("stop_loss_price") is not None
            assert status.get("take_profit_price") == 60000.0
    ...
    def test_hard_exit_time_in_execution(self):
        ...
        pos.entry_time = datetime.now(UTC) - timedelta(minutes=2)

        # After hard exit time - should exit
        should_exit, reason = manager.check_exits("time_exit_pos", 50500.0)
        assert should_exit is True
        assert reason is not None and "time" in reason.lower()
    ...
    def test_breakeven_protection_in_execution(self):
        ...
        manager.add_position(
            ...
            entry_price=50000.0,
            side="long",
            stop_config={"stop_loss_price": 45000.0, "breakeven_trigger_percent": 5.0},
        )
...
class TestStopExecutionWorkflow:
    ...
    def test_complete_long_trade_workflow(self):
        ...
        for price in price_movements:
            ...
            if should_exit:
                exit_triggered = True
                assert reason is not None and "take_profit" in reason.lower()
                break
