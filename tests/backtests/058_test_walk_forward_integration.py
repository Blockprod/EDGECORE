...
class TestSplitWalkForward:
    ...
    def test_split_creates_correct_period_count(self):
        ...
        data = pd.DataFrame(
            {"AAPL": np.random.randn(252).cumsum() + 100, "MSFT": np.random.randn(252).cumsum() + 50}, index=dates
        )
    ...
    def test_split_creates_train_larger_than_test(self):
        ...
        dates = pd.date_range("2024-01-01", periods=252, freq="D")
        data = pd.DataFrame({"AAPL": np.random.randn(252).cumsum() + 100}, index=dates)
    ...
    def test_split_with_different_oos_ratios(self):
        ...
        dates = pd.date_range("2024-01-01", periods=252, freq="D")
        data = pd.DataFrame({"price": np.random.randn(252).cumsum() + 100}, index=dates)
...
class TestWalkForwardBacktest:
    ...
    def test_walk_forward_aggregate_metrics(self):
        ...
        result = backtester.run_walk_forward(
            symbols=["AAPL", "MSFT"], start_date="2023-01-01", end_date="2023-12-31", num_periods=3, use_synthetic=True
        )
        ...
        assert agg["num_periods_completed"] == 3, f"Expected 3 completed periods, got {agg['num_periods_completed']}"
    ...
    def test_walk_forward_per_period_breakdown(self):
        ...
        result = backtester.run_walk_forward(
            symbols=["AAPL"], start_date="2023-01-01", end_date="2023-12-31", num_periods=2, use_synthetic=True
        )
...
class TestWalkForwardAntiDataLeakage:
    ...
    def test_train_test_no_temporal_overlap(self):
        ...
        for i, (train, test) in enumerate(splits):
            assert cast(pd.Timestamp, train.index.max()) < cast(pd.Timestamp, test.index.min()), (
                f"DATA LEAKAGE in split {i}: train ends {train.index.max()} but test starts {test.index.min()}"
            )
    ...
    def test_test_sets_are_strictly_forward(self):
        ...
        data = pd.DataFrame({"AAPL": np.random.randn(365).cumsum() + 100}, index=dates)

        splits = split_walk_forward(data, num_periods=5, oos_ratio=0.2)

        for i in range(1, len(splits)):
            prev_test_start = cast(pd.Timestamp, splits[i - 1][1].index.min())
            curr_test_start = cast(pd.Timestamp, splits[i][1].index.min())
    ...
    def test_no_future_data_in_training(self):
        ...
        for i, (train, test) in enumerate(splits):
            ...
            assert len(overlap) == 0, f"Split {i}: {len(overlap)} overlapping dates between train and test"
