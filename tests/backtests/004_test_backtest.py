...
def test_backtest_metrics_from_returns():
    ...
    trades = [100.0, -50.0, 150.0, -30.0, 200.0]  # Some winning, some losing

    metrics = BacktestMetrics.from_returns(
        returns=returns, trades=trades, start_date="2023-01-01", end_date="2023-01-08"
    )
...
def test_backtest_metrics_negative_returns():
    ...
    trades = [-100.0, -50.0, -150.0]  # All losing trades
...
def test_backtest_metrics_max_drawdown():
    ...
    metrics = BacktestMetrics.from_returns(
        returns=returns, trades=trades, start_date="2023-01-01", end_date="2023-01-06"
    )
...
def test_backtest_metrics_profit_factor():
    ...
    returns = pd.Series([0.01, 0.01, 0.01])
    trades = [1000.0, 500.0, -200.0, -100.0]  # Profit = 1500, Loss = 300
