...
class OutOfSampleValidator:
    ...
    def __init__(
        self, oos_acceptance_threshold: float = 0.70, hl_drift_tolerance: float = 0.50, num_symbols: int = 100
    ):
        """
    ...
    def validate_pair(
        ...
        if oos_is_cointegrated and len(oos_result["residuals"]) > 0:
            try:
                oos_half_life = half_life_mean_reversion(pd.Series(oos_result["residuals"]))
            except Exception:
                oos_half_life = None
    ...
    def _evaluate_validation(
        ...
        if is_half_life and oos_half_life:
            ...
            if hl_drift > self.hl_drift_tolerance:
                return False, (f"Half-life drifted {hl_drift:.0%} (IS={is_half_life:.1f}d OOS={oos_half_life:.1f}d)")

        # Passed all checks
        return True, f"Valid pair (IS p={is_pvalue:.2e} OOS p={oos_pvalue:.2e})"

    def validate_pair_set(self, pairs_with_data: list[dict[str, Any]]) -> dict[str, Any]:
        """
