...
class ThresholdDataGenerator:
    ...
    def simulate_trades(
        self, spread: pd.Series, entry_threshold: float, exit_threshold: float, lookback: int = 20
    ) -> dict[str, float]:
        """
        ...
        # Sharpe ratio (approximation ÔÇô 252 trading days/year for equities)
...
class ThresholdFeatureEngineer:
    ...
    def engineer_features(self, examples: list[TrainingExample]) -> tuple[pd.DataFrame, pd.Series, pd.Series]:
        """
...
class MLThresholdOptimizer:
    ...
    - RMSE < 0.3 (within 0.3¤â of true threshold)
    """

    def __init__(self):
        """Initialize optimizer."""
        self.entry_model = RandomForestRegressor(
            n_estimators=100, max_depth=8, min_samples_split=5, min_samples_leaf=2, random_state=42
        )
    ...
    def train(self, X: pd.DataFrame, y_entry: pd.Series, y_exit: pd.Series, test_size: float = 0.2) -> dict[str, Any]:
        """
        ...
        """
        # Split data ÔÇö temporal (chronological) split to avoid look-ahead bias
        ...
        return {"entry": self.entry_test_metrics, "exit": self.exit_test_metrics}
...
class AdaptiveThresholdManager:
    ...
    - Sprint 2.5: OOS validation gate ÔÇô ML auto-disabled if OOS degradation > 20%
    ...
    def __init__(self, default_entry: float = 2.0, default_exit: float = 0.5):
        ...
        self.optimizer: MLThresholdOptimizer | None = None
        self.feature_engineer: ThresholdFeatureEngineer | None = None
        self.ml_enabled: bool = True  # Sprint 2.5: can be disabled by OOS validation
        self._oos_rejection_reason: str = ""

    def set_model(self, optimizer: MLThresholdOptimizer, feature_engineer: ThresholdFeatureEngineer) -> None:
        """
    ...
    def get_thresholds(self, pair_key: str, **pair_characteristics) -> tuple[float, float]:
        """
