...
<<<<<<< HEAD
    ...
    hedge_ratio_drift: float | None = None
...
=======
    ...
    """Current hedge ratio β estimate"""
...
=======
    ...
    pairs_drifting: int
    """Pairs with significant β drift (drift >= 10%)"""
...
=======
        hedge_ratio_drift_threshold: Max acceptable β change % (default 0.10 = 10%)
        min_pair_age_days: Minimum days before consideration for removal (default 30)
    """
...
<<<<<<< HEAD
    ...
    def discover_cointegrated_pairs(
        self, price_data: pd.DataFrame, symbols: list[str], exclude_existing: bool = True
...
<<<<<<< HEAD
        ...
        if isinstance(price_data.index, pd.DatetimeIndex):
            ...
            recent_data = price_data[price_data.index >= cutoff_ts]
        else:
        ...
        for i, sym1 in enumerate(symbols):
            for sym2 in symbols[i + 1 :]:
                ...
                # Test cointegration
                try:
                    eg_result = engle_granger_test(pd.Series(y), pd.Series(x))
...
<<<<<<< HEAD
    ...
    def reestimate_hedge_ratios(
        self, price_data: pd.DataFrame, paired_symbols: list[tuple[str, str]]
...
=======
    ...
    def reestimate_hedge_ratios(
        ...
        Updates β estimates for tracked pairs using recent data window, checking for
        statistical degradation or significant drift from original values.
...
<<<<<<< HEAD
        ...
        logger.info("pair_reestimation_complete", pairs_reestimated=len(reestimated))
...
<<<<<<< HEAD
        ...
        for pair_key, metadata in self.tracked_pairs.items():
            ...
            if age_days < self.min_pair_age_days:
                is_valid = metadata.is_valid and (metadata.current_p_value or 1.0) < self.cointegration_threshold
            else:
                # Mature pair: must maintain cointegration and not degrade too much
                is_valid = (
                    metadata.is_valid
                    and (metadata.current_p_value or 1.0) < self.cointegration_threshold
                    and (metadata.hedge_ratio_drift or 0.0)
                    < self.hedge_ratio_drift_threshold * 2  # 2x threshold for mature
                )
...
<<<<<<< HEAD
    ...
    def generate_retraining_report(
        self,
        new_pairs: list[tuple[str, float, float]] | None = None,
        reestimated_pairs: dict[str, tuple[float, float]] | None = None,
    ) -> RetrainingReport:
        """
...
<<<<<<< HEAD
                drifting.append((pair_key, (metadata.hedge_ratio_drift or 0.0) * 100))
...
<<<<<<< HEAD
            pairs_stable=sum(
                1
                for m in self.tracked_pairs.values()
                if m.is_valid and (m.hedge_ratio_drift or 0.0) < self.hedge_ratio_drift_threshold
            ),
            pairs_drifting=len(drifting),
            avg_hedge_ratio_drift=float(avg_drift),
...
<<<<<<< HEAD
__all__ = ["PairDiscoveryMetadata", "RetrainingReport", "ModelRetrainingManager"]
=======
__all__ = [
