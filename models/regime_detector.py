...
class RegimeDetector:
    ...
    def __init__(
        ...
        self.spread_history: deque = deque(maxlen=lookback_window)
        ...
        if returns is None:
            if len(self.spread_history) > 0:
                ...
                if abs(prev_spread) > 1e-10:
                    returns = np.log(abs(spread / prev_spread)) if self.use_log_returns else (spread - prev_spread) / abs(prev_spread)
                else:
        ...
        if len(self.spread_history) > 1:
            ...
            for i in range(len(self.spread_history) - 1):
                ...
                if abs(prev_val) > 1e-10:
                    ret = np.log(abs(curr_val / prev_val)) if self.use_log_returns else (curr_val - prev_val) / abs(prev_val)
                else:
