...
- Pair discovery: 30+ seconds Ôåô 4-5 seconds (6x speedup)
- Signal generation: 500ms Ôåô 150ms (3x speedup)
...
class VectorizedSignalGenerator:
    ...
    Example: 50 pairs ├ù 252 days = 12,600 signal computations
    - Loop-based: ~500ms
    - Vectorized: ~150ms
    ...
    def generate_signals_batch(
        ...
        z_current = pd.Series(
            {pair: series.iloc[-1] if len(series) > 0 else 0.0 for pair, series in z_scores_dict.items()}
        )
    ...
    def compute_all_z_scores_vectorized(
        ...
        # Single DataFrame ÔÇö rolling operates on all columns at once
...
class S34PerformanceOptimizer:
    ...
    def __init__(self):
        ...
        self.signal_generator = VectorizedSignalGenerator(entry_z_threshold=2.0, exit_z_threshold=0.5)
        self.timings = {}
