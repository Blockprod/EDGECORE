...
IMPLEMENT:
  - get_open_orders() → fetch all active orders from Binance
...
FICHIER: execution/ccxt_engine.py
ENHANCE:
  - Wrap risky CCXT calls with @retry_with_backoff
...
FICHIER: execution/ccxt_engine.py
CHANGE:
  - api_key = SecretsManager.load_secret("EXCHANGE_API_KEY")
...
CHANGE:
  - After load_ccxt_data(), validate result
...
CONTAINS:
  ...
  - test_complete_paper_trading_mock()
    Mock CCXT + Data → Full trading loop → Verify positions
...
BENCHMARKS to optimize:
  ...
  - OHLCV loading (batch vs serial CCXT calls)
  - Z-score calculation (vectorized vs loop)

RUN:
  pytest tests/test_performance.py -v -m performance
  python -m cProfile -s cumulative main.py --mode backtest --symbols BTC/USDT
```
