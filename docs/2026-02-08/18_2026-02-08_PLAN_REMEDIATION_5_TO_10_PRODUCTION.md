...
**Problème:** `prod.yaml` a `use_sandbox: false` → Binance réel activé par accident  
...
**Problème:** `execution/ccxt_engine.py` line 59 has `# TODO: Remove sandbox restriction in production`

**Tâches:**
```
1. File: execution/ccxt_engine.py:50-65
   - Remove TODO comment
   - Replace with explicit check:
     if not self.config.use_sandbox:
         logger.critical("LIVE_TRADING_ENABLED", exchange=self.config.exchange)
```
...
2. python main.py --mode paper --symbols BTC/USDT 2>&1 | head -20
...
3. Modify main.py (8h)
   ...
   - Query Binance open orders
   - Compare local state vs Binance state
   - Alert if mismatch
...
- ✅ Mismatch between local state and Binance detected + alerted
...
**Acceptance criteria:**
- ✅ Startup compares local vs Binance state
...
# Or manual: python main.py --mode paper --symbols BTC/USDT, then Ctrl+C
...
def classify_exception(exc: Exception) -> ErrorCategory:
    ...
    elif isinstance(exc, ccxt.InsufficientBalance):
...
def load_market_data_for_symbols(symbols: List[str], loader: DataLoader) -> Dict[str, pd.DataFrame]:
    ...
    for symbol in symbols:
        try:
            df = loader.load_ccxt_data(exchange_name='binance', symbol=symbol)
...
class DataLoader:
    ...
    def load_ccxt_data(self, exchange_name: str, symbol: str, 
                      timeframe: str = "1d", validate: bool = True) -> pd.DataFrame:
        """Load OHLCV data with validation."""
        
        try:
            # Load from CCXT
            df = self._fetch_from_ccxt(exchange_name, symbol, timeframe)
...
**Acceptance criteria:**
- ✅ Every load_ccxt_data() call validates OHLCV
...
**Problème:** CircuitBreaker existe mais n'est pas utilisé pour CCXT calls

**Solution:** Wrap all broker API calls

#### 4.3.1 Apply Circuit Breaker to CCXT Engine

**Fichier:** `execution/ccxt_engine.py` (refactor, 40 LOC)
...
```python
def load_ccxt_data(self, exchange_name: str, symbol: str, ...) -> pd.DataFrame:
    """Load data with circuit breaker protection."""
    
    try:
        breaker = getattr(self, f"api_breaker_{exchange_name}", None)
        if not breaker:
            breaker = CircuitBreaker(f"data_load_{exchange_name}")
            setattr(self, f"api_breaker_{exchange_name}", breaker)
        
        df = breaker.call(
            self._fetch_from_ccxt_raw,
            exchange_name, symbol
        )
        return df
    
    except CircuitBreakerOpen as e:
        raise DataError(f"Exchange API down: {e}")
```

**Tâches:**
```
1. Refactor execution/ccxt_engine.py (10h)
   ...
   - Map CCXT exceptions to ErrorCategories

2. Refactor data/loader.py (8h)
   - Wrap CCXT fetch calls with circuit breaker
...
# - Edit test to mock CCXT failures
...
- ✅ Circuit breaker active for CCXT + Data loader
...
#   - execution/ccxt_engine.py (40% → add mocks)
...
**Objective:** Test full trading flow with real Binance testnet
...
class E2ETestSuite:
    ...
    def setup_testnet_account(self):
        """Create Binance testnet account with initial balance."""
        # API key from testnet.binance.vision
        # Initial balance: 10 BTC, 10 ETH
...
2. Setup Binance testnet account (2h)
   - Generate API keys
...
class PaperMode(ExecutionMode):
    ...
    def on_startup(self, context):
        logger.info("PAPER_MODE_STARTING", exchange=context.settings.execution.exchange)
...
- Execution (execution/base.py, ccxt_engine.py, ibkr_engine.py)
...
# Run
CMD ["python", "main.py", "--mode", "paper", "--symbols", "BTC/USDT", "ETH/USDT"]
```
...
[ ] Ran on Binance testnet for >= 1 hour
