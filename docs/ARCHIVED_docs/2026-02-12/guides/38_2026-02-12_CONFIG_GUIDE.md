...
settings.reload_symbols(["AAPL", "MSFT", "JPM"])
...
# Test top 5 equities
settings.reload_symbols(["AAPL", "MSFT", "GOOGL", "JPM", "WFC"])
runner.run(settings.trading_universe.symbols, ...)

# Test DeFi only
settings.reload_symbols(["META", "NFLX", "TSLA", "NVDA", "AMD"])
runner.run(settings.trading_universe.symbols, ...)
...
### TEST (10 symbols)
```yaml
- AAPL, MSFT                        (2 mega cap)
- GOOGL, JPM, WFC              (3 large cap)
- V, GS, MS            (3 large cap)
- AMZN, BAC                      (2 mid cap)
```
