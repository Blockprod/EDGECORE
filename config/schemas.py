...
class RiskConfigSchema(BaseModel):
    ...
    max_position_size: float = Field(
        ...
        gt=0,
        le=1.0,
        description="Max % of portfolio per position (0 < size <= 1.0)"
    )
    ...
    max_portfolio_heat: float = Field(
        ...
        ge=0,
        le=1.0,
        description="Max % of portfolio at risk (0 <= heat <= 1.0)"
    )
    ...
    max_loss_per_trade: float = Field(
        ...
        ge=0,
        lt=1.0,
        description="Max loss % per trade (0 <= loss < 1.0)"
    )
    ...
    max_drawdown_pct: float = Field(
        ...
        ge=0,
        lt=100,
        description="Max portfolio drawdown % (0 <= dd < 100)"
    )
    ...
    max_correlation: float = Field(
        ...
        ge=0,
        le=1.0,
        description="Max correlation between pairs (0 <= corr <= 1.0)"
    )
    
    position_timeout_minutes: int = Field(
        default=120,
        gt=0,
        description="Max position duration in minutes"
    )
    
    min_equity_usd: float = Field(
        default=1000.0,
        gt=0,
        description="Minimum equity to continue trading"
    )
...
class StrategyConfigSchema(BaseModel):
    ...
    fast_sma_period: int = Field(
        default=20,
        gt=0,
        le=500,
        description="Fast SMA period (candles)"
    )
    
    slow_sma_period: int = Field(
        default=50,
        gt=0,
        le=500,
        description="Slow SMA period (candles)"
    )
    
    # Position sizing
    position_qty_base: float = Field(
        default=1.0,
        gt=0,
        description="Base position quantity"
    )
    
    # Entry/exit parameters
    entry_threshold_std: float = Field(
        default=2.0,
        gt=0,
        description="Entry threshold in standard deviations"
    )
    
    exit_threshold_std: float = Field(
        default=1.0,
        gt=0,
    ...
    # Timeouts
    max_position_minutes: int = Field(
        default=120,
        gt=0,
        description="Max position hold time"
    )
...
class ExecutionConfigSchema(BaseModel):
    ...
    limit_order_timeout_seconds: int = Field(
        default=30,
        gt=0,
        description="Timeout for limit orders"
    )
    
    max_order_retries: int = Field(
        default=3,
        ge=1,
        description="Max retries for order submission"
    )
    ...
    cancel_timeout_seconds: int = Field(
        default=10,
        gt=0,
        description="Timeout for cancel operations"
    )
...
class DataSourceConfigSchema(BaseModel):
    ...
    feed_type: Literal["rest", "websocket"] = Field(
        default="rest",
        description="Data feed type"
    )
    
    ohlcv_interval_minutes: int = Field(
        default=5,
        gt=0,
        description="OHLCV candle interval"
    )
    
    max_lookback_candles: int = Field(
        default=500,
        gt=0,
        le=10000,
        description="Max historical candles to maintain"
    )
    
    buffer_size: int = Field(
        default=1000,
        gt=0,
        description="Data buffer size"
    )
    
    price_feed_timeout_seconds: int = Field(
        default=30,
        gt=0,
        description="Timeout for price feed updates"
    )
...
class AlerterConfigSchema(BaseModel):
    ...
    alert_on_position_opened: bool = Field(
        default=True,
        description="Alert when position opened"
    )
    
    alert_on_position_closed: bool = Field(
        default=True,
        description="Alert when position closed"
    )
    
    alert_on_strategy_error: bool = Field(
        default=True,
        description="Alert on strategy errors"
    )
    
    alert_deduplication_seconds: int = Field(
        default=60,
        ge=0,
        description="Seconds to deduplicate alerts"
    )
    
    max_alerts_per_minute: int = Field(
        default=60,
        gt=0,
        description="Rate limit for alerts"
    )
...
class BacktestConfigSchema(BaseModel):
    ...
    initial_equity: float = Field(
        default=10000.0,
        gt=0,
        description="Initial backtest equity"
    )
    ...
    commission_pct: float = Field(
        ...
        ge=0,
        le=10,
        description="Commission as % of trade value"
    )
...
class FullConfigSchema(BaseModel):
    ...
    risk: RiskConfigSchema = Field(
        default_factory=RiskConfigSchema,
        description="Risk engine configuration"
    )
    
    strategy: StrategyConfigSchema = Field(
        default_factory=StrategyConfigSchema,
        description="Strategy configuration"
    )
    
    execution: ExecutionConfigSchema = Field(
        default_factory=ExecutionConfigSchema,
        description="Execution engine configuration"
    )
    
    data_source: DataSourceConfigSchema = Field(
        default_factory=DataSourceConfigSchema,
        description="Data source configuration"
    )
    
    alerter: AlerterConfigSchema = Field(
        default_factory=AlerterConfigSchema,
        description="Alerter configuration"
    )
    
    backtest: Optional[BacktestConfigSchema] = Field(
        default=None,
        description="Backtest configuration (optional)"
    )
