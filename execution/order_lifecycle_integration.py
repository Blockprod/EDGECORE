...
class OrderLifecycleIntegration:
    ...
    def __init__(
        ...
        logger.info(
            "order_lifecycle_integration_initialized",
    ...
    def process_timeouts(self) -> int:
        ...
        """
        try:
            expired_orders, remediation_actions = self.order_mgr.check_for_timeouts()
            ...
            if cancelled_count > 0:
                logger.warning(
                    "orders_auto_cancelled_due_to_timeout",
                    cancelled_count=cancelled_count,
                    order_ids=expired_orders
                )
    ...
    def get_pending_orders_for_symbol(self, symbol: str) -> List[str]:
        ...
        return [
            oid for oid in self.orders_by_symbol[symbol]
            if oid in self.order_mgr.orders
            and self.order_mgr.orders[oid].status == OrderStatus.PENDING
        ]
    
    def get_order_age_seconds(self, order_id: str) -> Optional[float]:
        """
