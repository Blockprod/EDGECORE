...
class BrokerReconciler:
    ...
    def __init__(
        ...
        internal_positions: Dict[str, Dict[str, Any]] = None,
        internal_orders: Dict[str, Dict[str, Any]] = None,
    ...
    def reconcile_positions(
            ...
            if symbol in self.internal_positions:
                ...
                if diff > self.position_tolerance_units:
                    inconsistencies.append(
                        f"Position {symbol}: broker={broker_size}, internal={internal_size}"
                    )
                    divergence = ReconciliationDivergence(
                        type="position",
                        severity="medium",
        ...
        if matches:
            logger.info(
                "positions_reconciliation_ok",
                positions_count=len(broker_positions)
            )
        else:
            logger.warning(
                "positions_divergence_detected",
        ...
        for order_id, internal_order in self.internal_orders.items():
            if order_id not in broker_orders:
                ...
                if status not in ["filled", "cancelled"]:
                    inconsistencies.append(
                        f"Order {order_id} missing on broker but status is {status}"
                    )
    ...
    def full_reconciliation(
        ...
        positions_matches, position_inconsistencies = self.reconcile_positions(broker_positions)
        orders_matches, order_inconsistencies = self.reconcile_orders(broker_orders)
