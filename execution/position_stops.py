...
class PositionStop:
    ...
    trailing_stop_distance: Price | None = None
    hard_exit_time_minutes: int | None = None
    breakeven_trigger_percent: float | None = None

    # Tracking state
    trailing_high: Price | None = None  # For long positions
    trailing_low: Price | None = None  # For short positions
    ...
    def check_hard_exit(self) -> bool:
        ...
        elapsed = datetime.now(UTC) - self.entry_time
    ...
    def check_breakeven_protection(self, current_price: Price) -> bool:
        ...
        if profit_pct >= self.breakeven_trigger_percent:
            if (
                ...
                or (self.side == "long" and self.stop_loss_price < self.entry_price)
                or (self.side == "short" and self.stop_loss_price > self.entry_price)
            ):
    ...
    def get_status(self) -> PositionStopStatus:
        ...
        status: PositionStopStatus = PositionStopStatus(
            position_id=self.position_id,
            symbol=self.symbol,
        ...
        if self.stop_loss_price is not None:
            status["stop_loss_price"] = self.stop_loss_price
        if self.take_profit_price is not None:
            status["take_profit_price"] = self.take_profit_price
        if self.trailing_high is not None:
            status["trailing_high"] = self.trailing_high
        if time_to_exit is not None:
            status["time_to_hard_exit"] = time_to_exit
        return status
...
class PositionStopManager:
    ...
    def add_position(
        ...
        entry_price: Price,
        side: str,
        stop_config: PositionStopConfig | None = None,
    ) -> PositionStop:
        """
    ...
    def check_exits(self, position_id: PositionID, current_price: Price) -> tuple[bool, str | None]:
        """
    ...
    def get_status(self, position_id: PositionID) -> PositionStopStatus | None:
    ...
    def get_all_statuses(self) -> dict[PositionID, PositionStopStatus]:
        ...
        return {pos_id: pos_stop.get_status() for pos_id, pos_stop in self.positions.items()}


# Global instance
_stop_manager: PositionStopManager | None = None
