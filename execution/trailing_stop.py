...
Solution: Trailing stops that exit if spread widens > 1.0¤â from entry.

Mechanism:
- Record Z-score at entry
- Monitor current Z-score continuously
- Exit if spread widens by > 1.0¤â: |current_z| - |entry_z| > 1.0
...
class TrailingStopPosition:
    ...
    max_profit_z: float = None  # Best Z-score achieved (for tracking)
    max_loss_z: float = None  # Worst Z-score achieved
...
class TrailingStopManager:
    ...
    - Exit if spread widens > threshold (default: 1.0¤â)
    - Allows tighter stops as positions become profitable
    """
    
    def __init__(
        self,
        widening_threshold: float = 1.0,
        track_max_profit: bool = True
    ):
        """
        Initialize trailing stop manager.
        
        Args:
            widening_threshold: Exit if spread widens by this many ¤â from entry
                              (default 1.0¤â)
    ...
    def should_exit_on_trailing_stop(
        ...
        - If position entered at entry_z = 2.2 (long):
          ...
          - Current Z = 3.8 Ôåô widening = |3.8| - |2.2| = 1.6¤â
          - If 1.6 > threshold (1.0) Ôåô exit
        
        - If position entered at entry_z = -2.0 (short):
          - Expect reversion toward 0 (widening would be more negative)
          - Current Z = -3.5 Ôåô widening = |-3.5| - |-2.0| = 1.5¤â
          - If 1.5 > threshold (1.0) Ôåô exit
        ...
        if self.track_max_profit:
            ...
            if position.side == "long":
                position.max_profit_z = min(position.max_profit_z, current_z)
                position.max_loss_z = max(position.max_loss_z, current_z)
            else:  # short
                position.max_profit_z = max(position.max_profit_z, current_z)
                position.max_loss_z = min(position.max_loss_z, current_z)
        
        # Check trailing stop condition
        if widening > self.widening_threshold:
            exit_reason = (
                f"Trailing stop: spread widened {widening:.2f}¤â from entry "
    ...
    def should_exit_on_tight_trailing_stop(
        ...
        - Profit = 2.2 - 1.0 = 1.2¤â (profit!)
        - Use tighter threshold: e.g., 0.3¤â (if widens 0.3¤â from best, exit)
        ...
        if position.side == "long":
            ...
            best_achieved = abs(max_profit_z)
        else:
        ...
        if profit > profit_threshold:
            # Tighter threshold: 0.3¤â (vs normal 1.0¤â)
            tight_threshold = 0.3
            ...
            if widening_from_best > tight_threshold:
                exit_reason = (
                    ...
                    f"spread widened {widening_from_best:.2f}¤â from best "
    ...
    def get_position_info(self, symbol_pair: str) -> Dict:
        """
    ...
    def remove_position(self, symbol_pair: str) -> None:
        ...
        if symbol_pair in self.positions:
            ...
            logger.debug(
                ...
                max_profit_z=f"{info.max_profit_z:.3f}",
                max_loss_z=f"{info.max_loss_z:.3f}"
            )
            del self.positions[symbol_pair]
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            'avg_max_profit_z': np.mean([abs(p.max_profit_z) for p in positions_list]),
