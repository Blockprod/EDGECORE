...
class CircuitBreaker:
    ...
    def __init__(
        ...
        Args:
            ...
            probe_fn: Optional health-check called at OPEN→HALF_OPEN transition.
                      If it returns False the circuit stays OPEN and waits for the
                      next timeout cycle before retrying.
        """
        if not name:
    ...
    def opened_at(self, value: float):
        ...
        with self.lock:
            ...
            self.metrics.state_change_time = datetime.fromtimestamp(value, tz=UTC).replace(tzinfo=None)
