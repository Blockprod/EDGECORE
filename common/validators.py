...
def validate_position_size(position_size: float, min_size: float = 0.0001, 
                          max_size: float = 1_000_000.0) -> None:
    """
    ...
    if not isinstance(position_size, (int, float)):
        raise ValidationError(
            f"Position size must be numeric, got {type(position_size)}"
        )
    ...
    if position_size < min_size:
        raise ValidationError(
            f"Position size {position_size} below minimum {min_size}"
        )
    
    if position_size > max_size:
        raise ValidationError(
            f"Position size {position_size} exceeds maximum {max_size}"
        )
    
    logger.debug("position_size_validated", position_size=position_size)


def validate_equity(equity: float, min_equity: float = 100.0, 
                   max_equity: float = 1_000_000_000.0) -> None:
    """
    ...
    if equity < min_equity:
        raise EquityError(
            f"Equity {equity} suspiciously low (< {min_equity}). "
            f"Possible data corruption or broker API error."
        )
    
    if equity > max_equity:
        raise EquityError(
            f"Equity {equity} suspiciously high (> {max_equity}). "
            f"Possible data corruption or broker API error."
        )
...
def validate_volatility(volatility: float, min_vol: float = 0.0001, 
                       max_vol: float = 10.0) -> None:
    """
    ...
    if volatility < min_vol:
        raise VolatilityError(
            f"Volatility {volatility} implausibly low (< {min_vol}). "
            f"Possible calculation error."
        )
    
    if volatility > max_vol:
        raise VolatilityError(
            f"Volatility {volatility} implausibly high (> {max_vol}). "
            f"Possible data corruption or calculation error."
        )
    
    logger.debug("volatility_validated", volatility=volatility)


def validate_config(config: Dict[str, Any], schema: Optional[Dict] = None) -> None:
    """
    ...
    if 'risk' in config:
        ...
        if 'max_risk_per_trade' in risk:
            ...
            if not isinstance(mrt, (int, float)) or mrt <= 0 or mrt > 0.5:
                raise ConfigError(
                    f"max_risk_per_trade must be 0-50%, got {mrt*100}%"
                )
        ...
        if 'max_concurrent_positions' in risk:
            ...
            if not isinstance(mcp, int) or mcp < 1 or mcp > 1000:
                raise ConfigError(
                    f"max_concurrent_positions must be 1-1000, got {mcp}"
                )
        ...
        if 'max_daily_loss_pct' in risk:
            ...
            if not isinstance(mdl, (int, float)) or mdl <= 0 or mdl > 0.5:
                raise ConfigError(
                    f"max_daily_loss_pct must be 0-50%, got {mdl*100}%"
                )
    ...
    if 'execution' in config:
        ...
        if 'timeout_seconds' in execution:
            ...
            if not isinstance(ts, int) or ts < 1 or ts > 300:
                raise ConfigError(
                    f"timeout_seconds must be 1-300, got {ts}"
                )
...
def validate_trade_entry(symbol: str, position_size: float, equity: float, 
...
def validate_risk_parameters(max_risk_per_trade: float, 
    ...
    with SanityCheckContext("risk_parameters"):
        if not isinstance(max_risk_per_trade, (int, float)) or \
           max_risk_per_trade <= 0 or max_risk_per_trade >= 0.5:
            raise ValidationError(
                f"max_risk_per_trade must be 0-50%, got {max_risk_per_trade*100}%"
            )
        
        if not isinstance(max_concurrent_positions, int) or \
           max_concurrent_positions < 1 or max_concurrent_positions > 1000:
            raise ValidationError(
                f"max_concurrent_positions must be 1-1000, got {max_concurrent_positions}"
            )
        
        if not isinstance(max_daily_loss_pct, (int, float)) or \
           max_daily_loss_pct <= 0 or max_daily_loss_pct >= 0.5:
            raise ValidationError(
                f"max_daily_loss_pct must be 0-50%, got {max_daily_loss_pct*100}%"
            )
