...
def retry_with_backoff(policy: RetryPolicy | None = None, on_retry: Callable | None = None) -> Callable:
    """
            ...
            for attempt in range(policy.max_attempts):
                try:
                    ...
                    if attempt >= policy.max_attempts - 1:
                        ...
                        raise RetryException(f"Retry exhausted after {policy.max_attempts} attempts: {str(e)}") from e
...
class RetryStats:
    ...
    def record_call(self, func_name: str, success: bool, retries: int = 0, time_seconds: float = 0.0):
        ...
        if func_name not in self.by_function:
            self.by_function[func_name] = {"calls": 0, "successful": 0, "failed": 0, "total_retries": 0}
