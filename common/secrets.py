...
class MaskedString:
    ...
    def get_masked(self) -> str:
        ...
        masked = (
            self.value[:show_start] +
            "*" * (len(self.value) - show_start - show_end) +
            self.value[-show_end:]
        )
        return masked
...
class SecretsVault:
    ...
    def get_secret(self, name: str) -> str:
        ...
        if self._metadata[name].needs_rotation():
            logger.warning(
                ...
                secret_name=name,
                days_since_rotation=self._calculate_rotation_age(name)
            )
    ...
    def rotate_secret(
        ...
        # Audit log
        self._log_audit("rotate", name, success=True, details={
...
def mask_sensitive_data(text: str) -> str:
    ...
    for pattern in SecretsVault.SENSITIVE_PATTERNS:
        ...
        text = re.sub(
            rf'({pattern}\s*[:=]\s*["\']?)([^\s"\']+)(["\']?)',
            lambda m: m.group(1) + m.group(2)[0] + '*' * max(3, len(m.group(2)) - 2) + (m.group(2)[-1] if len(m.group(2)) > 1 else '') + m.group(3),
            text,
            flags=re.IGNORECASE
        )
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.warning(
                    "secret_not_found_for_decorator",
                    secret_name=secret_name,
