    ...
    def cluster_pairs(
        ...
        while unclustered:
            ...
            cluster_corr_matrix = corr_matrix[
                np.ix_(list(cluster_indices), list(cluster_indices))
            ]
            ...
            cluster = ClusterAnalysis(
                ...
                members_correlation={
                    str(pairs[i]): float(np.mean(cluster_corr_matrix[j]))
...
class PortfolioConcentrationAnalyzer:
    ...
    def analyze_concentration(
        self,
        pair_exposures: Dict[str, PairExposure]
    ) -> Dict[str, Any]:
        """
        ...
        # Calculate weights
        symbol_weights = {
            sym: abs(exp) / total_notional
            for sym, exp in total_exposure.items()
        }
    ...
    def get_rebalancing_adjustments(
        self,
        pair_exposures: Dict[str, PairExposure]
    ) -> Dict[str, float]:
        """
        ...
        for pair_id, exposure in pair_exposures.items():
            ...
            contributes_to_violation = any(
                sym in violation_symbols for sym in exp_dict.keys()
            )
...
class PortfolioManager:
    ...
    def register_pair(self, pair_id: str, sym1: str, sym2: str) -> None:
        ...
        with self._lock:
            if pair_id not in self.pair_exposures:
                self.pair_exposures[pair_id] = PairExposure(
                    pair_id=pair_id,
                    sym1=sym1,
                    sym2=sym2
                )
    ...
    def analyze_portfolio(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            pairs = [
                (exp.sym1, exp.sym2)
                for exp in self.pair_exposures.values()
            ]
            ...
            concentration = self.concentration.analyze_concentration(
                self.pair_exposures
            )
    ...
    def get_rebalancing_plan(self) -> Dict[str, Any]:
        ...
        """
        with self._lock:
            adjustments = self.concentration.get_rebalancing_adjustments(
                self.pair_exposures
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_cluster_size = (
                sum(len(c.pairs) for c in clusters) / len(clusters)
