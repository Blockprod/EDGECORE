...
class AlertManager:
    ...
    def __init__(self, max_alert_history: int = 10000):
        ...
        self.handlers: dict[AlertSeverity, list[Callable]] = {severity: [] for severity in AlertSeverity}
        self.category_handlers: dict[AlertCategory, list[Callable]] = {category: [] for category in AlertCategory}
    ...
    def create_alert(
        ...
        alert = Alert(
            alert_id=alert_id, severity=severity, category=category, title=title, message=message, data=data or {}
        )
        ...
        logger.info("alert_created", alert_id=alert_id, severity=severity.value, category=category.value, title=title)
    ...
    def register_severity_handler(self, severity: AlertSeverity, handler: Callable[[Alert], None]) -> None:
        """
    ...
    def register_category_handler(self, category: AlertCategory, handler: Callable[[Alert], None]) -> None:
        """
...
def alert_equity_drop(
    alert_manager: AlertManager, current_equity: float, previous_equity: float, threshold_pct: float = 5.0
) -> Alert | None:
    """
...
def alert_reconciliation_failure(
    alert_manager: AlertManager, divergence_count: int, equity_diff_pct: float
) -> Alert | None:
    """
...
def alert_position_limit_breach(
    alert_manager: AlertManager, current_positions: int, max_positions: int
) -> Alert | None:
    """
...
def alert_order_timeout(alert_manager: AlertManager, order_id: str, symbol: str, timeout_seconds: float) -> Alert:
    """
