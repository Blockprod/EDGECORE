    ...
    def _load_keys(self) -> set:
        ...
        if not keys:
            ...
            if os.getenv("EDGECORE_ENV") == "prod":
                logger.warning(
                    "API_KEYS_NOT_CONFIGURED", message="API_KEYS environment variable not set - dashboard is open!"
                )
...
# Global instances
_rate_limiter = RateLimiter(requests_per_minute=int(os.getenv("RATE_LIMIT_RPM", "100")))
...
_auth = APIKeyAuth(required=os.getenv("API_AUTH_REQUIRED", "false").lower() == "true")
...
def require_jwt_token(func: Callable) -> Callable:
    ...
    def wrapper(*args: Any, **kwargs) -> Any:
        if not _jwt_auth:
            return jsonify({"error": "JWT not configured", "message": "JWT authentication not available"}), 401
        ...
        if len(parts) != 2 or parts[0].lower() != "bearer":
            return jsonify({"error": "Invalid authorization header", "message": 'Expected "Bearer TOKEN"'}), 401
    ...
    def wrapper(*args, **kwargs) -> Any:
        if os.getenv("REQUIRE_HTTPS", "false").lower() == "true":
            if not request.is_secure:
                return jsonify({"error": "HTTPS required", "message": "This endpoint requires HTTPS"}), 403
...
class RequestLogger:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total": total_requests, "avg_time_ms": avg_time, "status_codes": status_counts}
...
def log_api_call(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            _request_logger.log_request(request.method, request.path, user_id, status_code, elapsed_ms)

            return result
        except Exception:
            elapsed_ms = (time.time() - start_time) * 1000
            _request_logger.log_request(request.method, request.path, user_id, 500, elapsed_ms)
            raise
...
def get_jwt_auth() -> JWTAuth | None:
    """Get JWT auth instance (if configured)."""
    return _jwt_auth


def generate_jwt_token(user_id: str, expires_in_hours: int = 24) -> str | None:
    """
