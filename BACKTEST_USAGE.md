...
### Default: 46 Major Cryptocurrencies (Configured in code)
```bash
# Just run with NO symbol arguments - uses 46 cryptos from config/dev.yaml
...
```bash
# Run with just BTC/ETH (small universe test)
...
Edit `config/dev.yaml` to modify which cryptos to trade:
...
- Remove symbols that don't exist on your exchange
...
1. **Loads data** for all 46 cryptocurrencies from Binance
...
### Quick Test (2023 data, 46 cryptos)
```bash
python main.py --mode backtest
```
...
```bash
python main.py --mode backtest --symbols BTC/USDT ETH/USDT LTC/USDT SOL/USDT LINK/USDT
```
...
3. **Updated** `main.py`:
   - Removed hardcoded `["BTC/USDC", "ETH/USDC"]`
...
4. Consider expanding to 100+ cryptos for even better coverage
